...
\title{Supplementary material to: Drug sensitivity prediction with normal 
inverse Gaussian shrinkage informed by external data}
\date{\today}
...
\begin{document}
  ...
  $g_d \cdot \trace[\diag(b_{jd})\bSigma_d]$, and $g_d \cdot \bmu_d \tr 
  \diag(b_{jd}) \bmu_d$.
  ...
  MCMC samples from the posterior corresponding to MD model 
  (2) may be generated for each equation independently.
  ...
  \begin{subequations}
    \begin{align}
      ...
      \(\frac{n + p + 1}{2}, \zeta_d\), \nonumber \\
      \text{with } & \zeta_d = \frac{1}{2} \bigg[\mathbf{y} \tr \mathbf{y} -
      2 \mathbf{y} \tr \X \bm{\mu} + \trace ( \X \tr \X \bm{\Sigma}) + 
      (\bm{\mu}) \tr \X \tr \X \bm{\mu} + 
      \tau^{-2}_d \trace (\diag(\gamma_{jd}^{-2}) \bm{\Sigma}) \nonumber \\
      & \,\,\,\,\,\,\,\,\,\, + \tau_d^{-2} (\bm{\mu}) \tr 
      \diag(\gamma_{jd}^{-2}) \bm{\mu}\bigg]. \nonumber
    \end{align}
  \end{subequations}
