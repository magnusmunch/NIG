...
\begin{document}
  ...
  with vague (improper) hyperpriors (as in \cite{upadhyay_bayesian_1996}:
  \begin{align*}
    \bm{\alpha}_{\text{feat}} & \sim c_1 \\
    \bm{\alpha}_{\text{drug}} & \sim c_2, \\
    \lambda_{\text{feat}} &\sim 1/\lambda_{\text{feat}}, \\
    \lambda_{\text{drug}} &\sim 1/\lambda_{\text{drug}},
  \end{align*}
  where $c_1$ and $c_2$ are constants. The corresponding full conditionals are
  given by:
  \begin{align*}
    \bm{\alpha}_{\text{feat}} | \lambda_{\text{feat}}, \bm{\Gamma} & \sim 
    \mathcal{N} \([\mathbf{C} \tr \diag(\gamma_{jd}^2) \mathbf{C}]^{-1}
    \mathbf{C} \tr \mathbf{1}_{pD \times 1}, \lambda_{\text{feat}}^{-1} 
    [\mathbf{C} \tr \diag(\gamma_{jd}^2) \mathbf{C}]^{-1} \) \\
    \bm{\alpha}_{\text{drug}} | \lambda_{\text{drug}}, \bm{\tau} & \sim 
    \mathcal{N} \([\mathbf{Z} \tr \diag(\tau_{d}^2) \mathbf{Z}]^{-1}
    \mathbf{Z} \tr \mathbf{1}_{D \times 1}, \lambda_{\text{drug}}^{-1} 
    [\mathbf{Z} \tr \diag(\tau_{d}^2) \mathbf{Z}]^{-1} \), \\
    \lambda_{\text{feat}} | \bm{\alpha}_{\text{feat}}, \bm{\Gamma} & 
    \sim \Gamma \(\frac{pD}{2}, \frac{1}{2} 
    \[\mathbf{C} \bm{\alpha}_{\text{feat}} - \diag(\gamma_{jd}^{-2}) 
    \mathbf{1}_{pD \times 1}\] \tr \diag(\gamma_{jd}^2) 
    \[\mathbf{C} \bm{\alpha}_{\text{feat}} - 
    \diag(\gamma_{jd}^{-2}) \mathbf{1}_{pD \times 1}\] \), \\
    \lambda_{\text{drug}} | \bm{\alpha}_{\text{drug}}, \bm{\tau} & 
    \sim \Gamma \(\frac{D}{2}, \frac{1}{2} 
    \[\mathbf{Z} \bm{\alpha}_{\text{drug}} - \diag(\tau_{d}^{-2}) 
    \mathbf{1}_{D \times 1}\] \tr \diag(\tau_{d}^2) 
    \[\mathbf{Z} \bm{\alpha}_{\text{drug}} - 
    \diag(\tau_{d}^{-2}) \mathbf{1}_{D \times 1}\] \).
  \end{align*}
