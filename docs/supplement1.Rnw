...
\begin{document}
  ...
  Consider the following simpler alternative prior to 
  ...
  \begin{subequations}\label{eq:altprior}
	...
	variance follows the Jeffreys prior from (\ref{md-eq:prior}) in the MD. 
	Throughout the
  ...
  parameter estimate $\bm{\alpha}^{(l)}$ with function 
  $g(\bm{\alpha} | \bm{\alpha}^{(l)})$, such that
  $\forall \bm{\alpha}: g(\bm{\alpha} | \bm{\alpha}^{(l)}) \leq 
  \log p(\bm{\alpha} | \mathbf{Y})$ 
  and $g(\bm{\alpha}^{(l)} | \bm{\alpha}^{(l)}) = 
  \log p(\bm{\alpha}^{(l)} | \mathbf{Y})$. Then, the sequence
  $$
  \bm{\alpha}^{(l + 1)} = \underset{\bm{\alpha}}{\argmax} 
  g(\bm{\alpha} | \bm{\alpha}^{(l)})
  $$
  converges to a local maximum of $\log p(\bm{\alpha} | \mathbf{Y})$. 
  Generally, the minorisation
  ...
  \begin{subequations}\label{eq:minor}
    \begin{align}
      g(\bm{\alpha} | \bm{\alpha}^{(l)}) = -\sum_{d=1}^D \bigg[
      & \diag \( \X \tr \mathbf{A}_d^{(l)} \X \) \tr \exp (\tilde{\mathbf{C}}_d
      \bm{\alpha}) \\
      & + \frac{n + 1}{\mathbf{y}_d \tr \mathbf{A}_d^{(l)} \mathbf{y}_d} 
      \mathbf{y}_d \tr \left\{ \I + \mathbf{X} \diag \[ \exp\( 
      \tilde{\mathbf{C}}_d \bm{\alpha} \) \] \mathbf{X} \tr \right\}^{-1} 
      \mathbf{y}_d\bigg],
    \end{align}
  \end{subequations}
  where $\mathbf{A}_d^{(l)} = \left\{ \I + \mathbf{X} \diag \[ \exp\( 
  \tilde{\mathbf{C}}_d \bm{\alpha}^{(l)} \) \] \mathbf{X} \tr \right\}^{-1}$.
  ...
  \mathbf{y}_d \tr \left\{ \I + \mathbf{X} \diag \[ \exp\( 
  \tilde{\mathbf{C}}_d \bm{\alpha}^{(l)} \) \] \mathbf{X} \tr \right\}^{-1} 
  \mathbf{y}_d = \underset{\bm{\delta}_d}{\max} 
  - ||\mathbf{y}_d - \mathbf{X} \bm{\delta}_d ||_2^2 - 
  \bm{\delta}_d \tr \diag \[ \exp\(
  -\tilde{\mathbf{C}}_d \bm{\alpha}^{(l)} \) \] \bm{\delta}_d.
  $$
  The extra parameters $\bm{\delta}_d$ are estimated per drug and by any 
  standard ridge regression package, such as \texttt{glmnet} in \texttt{R}. 
...
rownames(tab1) <- c("NIG$_{\\text{f}}^-$", 
                     "NIG$_{\\text{f}}$", "ridge", "lasso", "mxtune")
...
colnames(tab2) <- c("$\\text{EMSE}_{\\text{total}}$", 
                    "$\\text{EMSE}_{\\text{top}}$", 
                    "$\\text{EMSE}_{\\text{bottom}}$", "$\\text{PMSE}$")     
rownames(tab2) <- c("NIG$_{\\text{d}}^-$", 
                     "NIG$_{\\text{d}}$", "ridge", "lasso")
...
colnames(tab3) <- c("$\\text{EMSE}_{\\text{total}}$", 
                    "$\\text{EMSE}_{\\text{top}}$", 
                    "$\\text{EMSE}_{\\text{bottom}}$", "$\\text{PMSE}$")     
rownames(tab3) <- c("NIG$_{\\text{d}}^-$", 
                     "NIG$_{\\text{d}}$", "ridge", "lasso", "mxtune")
@	
	Tables \ref{tab:simulations_gdsc_tab1}-\ref{tab:simulation_gdsc_tab3} 
	...
	Scenarios 1-4. PMSE is calculated as in Section \ref{md-sec:gdsc} of the MD, 
  ...
  models, that provide full posteriors, the posterior mean 
...
kableExtra::kable_styling(knitr::kable(
  tab1, align="r",
  caption="Mean EMSE (split for bottom 10\\% and top 90\\% of $|\\hat{\\beta}_{jd}|$) and PMSE for simulation Scenario 1, estimated on test data (lowest in bold).", 
...
kableExtra::kable_styling(knitr::kable(
  tab2, align="r",
  caption="Mean EMSE (split for bottom 10\\% and top 90\\% of $|\\hat{\\beta}_{jd}|$) and PMSE for simulation Scenario 2, estimated on test data (lowest in bold).", 
  format="latex", booktabs=TRUE, escape=FALSE), 
  latex_options=c("HOLD_position"))
@
...
<<simulations_gdsc_pmse4, fig.cap="Simulation results for Scenario 4: Average PMSE against the proportion of permuted rows in the external features.", out.width="100%", fig.asp=1>>= 
@

	Focussing on EMSE, we see that the NIG$_{\text{f}}$ model,
