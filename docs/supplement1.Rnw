...
\begin{document}
	...
	used for rate $\lambda$: $\lambda=1$ (solid), $\lambda=0.2$ (dashed), and 
	$\lambda=0.1$ (dotted).
	...
	variance, i.e., $y_i \sim \mathcal{N} (\mu, \sigma^2)$. This gives us the
	...
	\end{bmatrix}
	$$
	Jeffrey's prior is now found as:
	$$
  ...
  \begin{align*}
    ...
    \end{cases}
  \end{align*} 
  The ratio $K_0(x)/K_1(x)$ is well-behaved, so that it may be computed
  ...
  Next, we note that through Jensen's inequality 
  ...
  Sampling from (\ref{eq:gibbssampler}) either requires specifying 
  hyperparameters
  ...
  rarely available, detailed subject knowledge, so we resort to endowing them
  with possibly flat, improper (as in \cite{upadhyay_bayesian_1996})
  hyperpriors:
  \begin{align*}
  ...
  $\xi_{\text{drug}}$. Improper priors occur if we let 
  ...
  \section{Multivariate Bayesian extension of \texttt{xtune}}\label{sec:mxtune}
  Consider the following simpler hyperprior alternative to 
  equation (\ref{md-eq:prior}) in the MD:
  \begin{subequations}\label{eq:altprior}
		\begin{align}
		  \beta_{jd} | \gamma^2_{jd}, \tau_d^2, \sigma^2_d & \sim \mathcal{N}_{p} 
		  (0, \gamma_{jd}^2 \tau_d^2 \sigma_d^2), \\
		  \log \gamma_{jd}^{2} & = \mathbf{c}_{jd} \tr \bm{\alpha}_{\text{feat}}, \\
		  \log \tau_d^{2} & = \mathbf{z}_{d} \tr \bm{\alpha}_{\text{drug}}\\
		  \sigma_d^{2} & \sim \Gamma^{-1} (\nu, \zeta).
		\end{align}
	\end{subequations}
	This model is a Bayesian, multivariate extension of 
	\cite{zeng_incorporating_2020}. The model does 
	not propagate uncertainty in the variance components $\gamma_{jd}^{2}$
	and $\tau_d^{2}$, but directly models the variance components as an
	exponentiated linear function of the external covariates. The linear functions
	are modelled on the log-scale to ensure non-negative variance components.
	Additionally, it allows for more flexibility in the error variance
	prior model. Note that in the limit $\zeta \to 0$ and $\nu=1/2$, the error
	variance follows the Jeffrey`s prior from (\ref{md-eq:prior}) in the MD. 
	Throughout the
	following, whenever $\zeta$ and $\nu$ are not specified, we use $\zeta \to 0$ 
	and $\nu=1/2$.
	
  The hyperparameters $\bm{\alpha} = \begin{bmatrix} \bm{\alpha}_{\text{feat}}
  \tr & \bm{\alpha}_{\text{drug}} \tr \end{bmatrix} \tr$ in model 
  (\ref{eq:altprior}) are estimated by empirical Bayes maximisation of
  the marginal log likelihood:
  \begin{subequations}\label{eq:altmll}
    \begin{align}
      \log p (\bm{\alpha} | \mathbf{Y}) & = \sum_{d=1}^D
      \log \[ \int_{\bm{\beta}_d} \int_{\sigma^2_d} \mathcal{L} 
      (\mathbf{y}_d | \bm{\beta}_d, \sigma^2_d ) \pi(\bm{\beta}_d | \sigma^2_d) 
      \pi(\sigma^2_d) d\bm{\beta}_d d\sigma_d^2 \] \\
      & \propto - \sum_{d=1}^D \bigg[ \log \left|\I + \mathbf{X} 
      \diag \[ \exp\( \tilde{\mathbf{C}}_d \bm{\alpha} \) \] \mathbf{X} \tr 
      \right| \\
      & \,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\, + (n + 1) \log \( \mathbf{y}_d \tr 
      \left\{ \I + \mathbf{X} 
      \diag \[ \exp\( \tilde{\mathbf{C}}_d \bm{\alpha} \) \] \mathbf{X} 
      \tr\right\}^{-1} \mathbf{y}_d \) \bigg],
    \end{align}
  \end{subequations}
  where $\tilde{\mathbf{C}}_d = \begin{bmatrix} \mathbf{C}_d & 
  \mathbf{1}_{p \times 1} \cdot \mathbf{z}_d \tr \end{bmatrix}$, and
  $\mathbf{C}_d = \begin{bmatrix} \mathbf{c}_{1d} & \cdots & 
  \mathbf{c}_{pd} \end{bmatrix} \tr$.
  The first term in the sum of (\ref{eq:altmll}) is concave in 
  $\bm{\alpha}$, while the second term is convex in $\bm{\alpha}$. This renders
  the marginal log likelihood a sum of non-concave (as well as non-convex) 
  functions. General purpose optimisers, such as \texttt{optim} in \texttt{R} 
  are generally not well suited to maximise non-concave functions. Maximisation
  is therefore done through a minorise-maximisation (MM) algorithm, 
  similar to \cite{zeng_incorporating_2020}.
  
  The MM algorithm seeks to minorise the marginal log likelihood,
  $\log p(\bm{\alpha} | \mathbf{Y})$, at current
  parameter estimate $\bm{\alpha}^{(t)}$ with function 
  $g(\bm{\alpha} | \bm{\alpha}^{(t)})$, such that
  $\forall \bm{\alpha}: g(\bm{\alpha} | \bm{\alpha}^{(t)}) \leq 
  \log p(\bm{\alpha} | \mathbf{Y})$ 
  and $g(\bm{\alpha}^{(t)} | \bm{\alpha}^{(t)}) = 
  \log p(\bm{\alpha} | \mathbf{Y})$. Then, the sequence
  $$
  \bm{\alpha}^{(t + 1)} = \underset{\bm{\alpha}}{\argmax} 
  g(\bm{\alpha} | \bm{\alpha}^{(t+1)})
  $$
  converges to a local maximum of $f(\bm{\alpha})$. Generally, the minorisation
  function is chosen such that it is concave and easy to maximise. In our case,
  a suitable minorisation is:
  \begin{subequations}\label{eq:minor}
    \begin{align}
      g(\bm{\alpha} | \bm{\alpha}^{(t+1)}) = -\sum_{d=1}^D \bigg[
      & \diag \( \X \tr \mathbf{A}_d^{(t)} \X \) \tr \exp (\tilde{\mathbf{C}}_d
      \bm{\alpha}) \\
      & + \frac{n + 1}{\mathbf{y}_d \tr \mathbf{A}_d^{(t)} \mathbf{y}_d} 
      \mathbf{y}_d \tr \left\{ \I + \mathbf{X} \diag \[ \exp\( 
      \tilde{\mathbf{C}}_d \bm{\alpha} \) \] \mathbf{X} \tr \right\}^{-1} 
      \mathbf{y}_d\bigg],
    \end{align}
  \end{subequations}
  where $\mathbf{A}_d^{(t)} = \left\{ \I + \mathbf{X} \diag \[ \exp\( 
  \tilde{\mathbf{C}}_d \bm{\alpha}^{(t)} \) \] \mathbf{X} \tr \right\}^{-1}$.
  Note that (\ref{eq:minor}) is concave. However, it is slow to optimise. 
  In \cite{zeng_incorporating_2020}, another minorization is proposed for
  the second term:
  $$
  \mathbf{y}_d \tr \left\{ \I + \mathbf{X} \diag \[ \exp\( 
  \tilde{\mathbf{C}}_d \bm{\alpha} \) \] \mathbf{X} \tr \right\}^{-1} 
  \mathbf{y}_d = \underset{\delta}{\max} 
  - ||\mathbf{y}_d - \mathbf{X} \delta ||_2^2 - 
  \delta \tr \diag \[ \exp\(
  -\tilde{\mathbf{C}}_d \bm{\alpha} \) \] \delta.
  $$
  The extra parameter $\delta$ is estimated by any standard ridge regression
  package, such as \texttt{glmnet} in \texttt{R}. 
  
  \section{Extra simulation results}\label{sec:simulations}
  The lasso and ridge models were fit separately per drug, using the \texttt{R}
	package \texttt{glmnet} \cite[]{friedman_regularization_2010} with cross
	validated penalty parameters. The \texttt{xtune} model is fit with the 
	corresponding \texttt{R} package \cite[]{ni_bayesian_2019}.
	
	Performance measures are estimated from the remaining 914 cell lines
...
tab <- cbind(
  apply(
    aggregate(res[substr(rownames(res), 1, 6)=="emsel.", ], 
              by=list(rownames(res[substr(rownames(res), 1, 6)=="emsel.", ])),
              FUN="mean")[, -1], 2, median),
  apply(
    aggregate(res[substr(rownames(res), 1, 6)=="emseh.", ], 
              by=list(rownames(res[substr(rownames(res), 1, 6)=="emseh.", ])),
              FUN="mean")[, -1], 2, median),
  apply(
    aggregate(res[substr(rownames(res), 1, 5)=="emse.", ], 
              by=list(rownames(res[substr(rownames(res), 1, 5)=="emse.", ])),
              FUN="mean")[, -1], 2, median),
  apply(
    aggregate(res[substr(rownames(res), 1, 5)=="pmse.", ], 
              by=list(rownames(res[substr(rownames(res), 1, 5)=="pmse.", ])),
              FUN="mean")[, -1], 2, median))
...
@	
  ...
  $\lambda_{\text{feat}}$ is overestimated with median estimate 
	\Sexpr{round(est.lambdaf[2], 2)} (the
	true value is 1). The overestimated $\lambda_{\text{feat}}$ translates 
	into underestimated $\gamma_{jd}^2$ variances. 
...
@	
  ...
  $\lambda_{\text{feat}}$ is overestimated with median estimate
	...
	again underestimates the 
	prior $\gamma_{jd}^2$ variance.
	
	Median $\lambda_{\text{drug}}=\Sexpr{round(est.lambdad[2], 2)}$ is
...
@
  ...
  Median $\lambda_{\text{feat}}$ and $\lambda_{\text{drug}}$ are
...
<<simulations_gdsc_post5, fig.cap="Simulation results for Scenario 3: Samples from posterior with variational posterior superimposed (red line) for estimated hyperparameters (top row) $\\hat{\\chi}_1$, (second row) $\\hat{\\chi}_2$, (third row) $\\hat{\\chi}_3$, (bottom row) $\\hat{\\chi}_4$, (left column) $\\hat{\\phi}_1$, (second column) $\\hat{\\phi}_2$, (third column) $\\hat{\\phi}_3$, and (last column) $\\hat{\\phi}_4$.", out.width="100%", fig.asp=1>>= 
@

<<simulations_gdsc_pmse4, fig.cap="Simulation results for Scenario 4: Median PMSE against the proportion of permuted rows in the external features.", out.width="50%", fig.asp=1>>= 
@

  \section{Extra GDSC data results}\label{sec:gdsc}
  $$
  \text{CPO}_{id} = p(y_i | \bm{y}_{-i}) = 
  $$
  
<<analysis_gdsc_res2, fig.cap="Simulation results for Scenario 4: Median PMSE against the proportion of permuted rows in the external features.", out.width="50%", fig.asp=1>>= 
@
