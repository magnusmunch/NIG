...
\begin{document}
	...
	\section{NIG and ENIG prior}\label{sec:nigprior}
	...
	\section{DGIG prior}\label{sec:dnigprior}
	Equivalently, we may set 
  $\eta_{jd}^2=\tau_j^2 \gamma_d^2/(\theta_j^z \theta_d^c)$ and write 
  MD equation (\ref{md-eq:extendedmodel1}) as:
  \begin{align*}
  ...
  \begin{subequations}\label{eq:extendedmodel1.2}
    \begin{align}
      \beta_{jd} | \eta_{jd}^2, \sigma_d^2 & \sim \mathcal{N} 
    ...
    \end{align}
  \end{subequations}
	From (\ref{eq:extendedmodel1.2}), for a fixed error variance
	$\sigma_d^2$, we find the marginal prior variance and kurtosis:
	\begin{subequations}\label{eq:varkurtdgig}
	  \begin{align}
  ...
  \rightarrow 3(\theta_j^z/\lambda_j^z + 1)$. From (\ref{eq:varkurtdgig}) we see
  ...
  $\lambda_j^z$. This makes the model MD equations (\ref{md-eq:extendedmodel1})
  a flexible model.
  ...
  \subsection{DNIG and ENIG models}
  If we have the following forms:
  \begin{align*}
    \bm{\Sigma} = a^{-1} \(\tilde{\X} \tr \tilde{\X} + 
    \tilde{\mathbf{H}}\)^{-1}, \\
    \bm{\mu} = \(\tilde{\X} \tr \tilde{\X} + 
    \tilde{\mathbf{H}}\)^{-1} \tilde{\X} \tr \y,
  \end{align*}
  with $\tilde{\X} = \begin{bmatrix} \mathbf{1} & \X \end{bmatrix}$ and
  $$
  \tilde{\mathbf{H}} = 
  \begin{bmatrix} 
    0 & \mathbf{0} \\
  ...
  with
  \begin{align*}
    \mathbf{D} & = (\I_n + \X \H^{-1} \X \tr), \\
    q & = \[ n - \mathbf{1} \X \H^{-1} \X \tr \mathbf{1} + 
    \mathbf{1} \X \H^{-1} \X \tr \mathbf{D}^{-1} 
    \X \H^{-1} \X \tr \mathbf{1}\]^{-1} \\
    \mathbf{r} & = \diag( \H^{-1}) - \left\{ \[\H^{-1} \X \tr 
    \mathbf{D}^{-1}\] \circ (\H^{-1} \X \tr) \right\} \mathbf{1} +
    q (\H^{-1} \X \tr \mathbf{1}) \circ (\H^{-1} \X \tr \mathbf{1}) \\
    & \,\,\,\,\, - 2q \[\H^{-1} \X \tr \mathbf{D}^{-1} 
    \X \H^{-1} \X \tr \mathbf{1}\] \circ (\H^{-1} \X \tr \mathbf{1}) \\
    & \,\,\,\,\, + q \[\H^{-1} \X \tr \mathbf{D}^{-1} 
    \X \H^{-1} \X \tr \mathbf{1}\] \circ \[\H^{-1} \X \tr 
    \mathbf{D}^{-1} \X \H^{-1} \X \tr \mathbf{1}\].
  \end{align*}
  Likewise we find 
  \begin{align*}
    \bm{\mu} =  
    \begin{bmatrix}
      q \sum_{i=1}^n y_i - q t + qs \\
      \mathbf{u}
    \end{bmatrix},
  \end{align*}
  with
  \begin{align*}
    s & = 1 \X \H^{-1} \X \tr \mathbf{D}^{-1} \X \tr \H^{-1}
    \X \tr \y \\
    t & = \mathbf{1} \X \H^{-1} \X \tr \y \\
    \mathbf{u} & = q \(t - \sum_{i=1}^n y_i - 2s \) \H^{-1} \X \tr \mathbf{1} + 
    + \H^{-1} \X \tr \y + q \( \sum_{i=1}^n y_i + s\) 
    \H^{-1} \X \tr \mathbf{D}^{-1} \X \H^{-1} \X \tr \mathbf{1} \\
    & \,\,\,\,\, - \H^{-1} \X \tr \mathbf{D}^{-1} \X \H^{-1} \X \tr \y.
  \end{align*}
  We have
  \begin{align*}
    \X \bmu & = q \( \sum_{i=1}^n y_i + s - t \) \mathbf{1} + 
    q \(t - \sum_{i=1}^n y_i - 2s \) \X \H^{-1} \X \tr \mathbf{1} +
    \X \H^{-1} \X \tr \y \\
    & \,\,\,\,\, + q \(s + \sum_{i=1}^n \) \X \H^{-1} \X \tr \mathbf{D}^{-1} 
    \X \H^{-1} \X \tr \mathbf{1} - \X \H^{-1} \X \tr \mathbf{D}^{-1} 
    \X \H^{-1} \X \tr \y.
  \end{align*}
  Furthermore
  \begin{align*}
    \bSigma \X \tr & = q \( \sum_{i=1}^n y_i + s - t \) \mathbf{1} + 
    q \(t - \sum_{i=1}^n y_i - 2s \) \X \H^{-1} \X \tr \mathbf{1} +
    \X \H^{-1} \X \tr \y \\
    & \,\,\,\,\, + q \(s + \sum_{i=1}^n \) \X \H^{-1} \X \tr \mathbf{D}^{-1} 
    \X \H^{-1} \X \tr \mathbf{1} - \X \H^{-1} \X \tr \mathbf{D}^{-1} 
