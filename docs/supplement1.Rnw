...
@	
  ...
  $\E(\beta_{jd} | \mathbf{y}_d)$ is used as point estimate.
	...
	Focussing on estimation, we see that the NIG$_{\text{f}}$ model,
...
<<simulations_gdsc_pmse4, fig.cap="(a) Mean EMSE, separated for (b) bottom and (c) top 10\\% of $|\\hat{\\beta}_{jd}|$, and (d) PMSE for simulation Scenario 3 versus the proportion of permuted  rows in the external data, estimated on test data.", out.width="100%", fig.asp=1>>= 
@
...
<<analysis_gdsc_res2, fig.cap="Simulation results for Scenario 4: Median PMSE against the proportion of permuted rows in the external features.", out.width="100%", fig.asp=1>>= 
@
