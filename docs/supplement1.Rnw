...
\title{Supplementary material to: Drug sensitivity prediction with the normal 
inverse Gaussian prior informed by external data}
\date{\today}
...
\begin{document}
	...
	\begin{align*}
	  ...
	  \frac{3\phi}{\lambda} + 3,
	\end{align*}
	where $\mathcal{K}(\cdot)$ denotes the kurtosis.
