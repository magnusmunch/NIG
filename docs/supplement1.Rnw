...
\begin{document}
  ...
  \section{Alternative prior}\label{sec:altprior}
  ...
  The lasso and ridge models were fit separately per drug, using the \texttt{R}
	...
	validated
	penalty parameters. The \texttt{xtune} model is fit with the corresponding 
	\texttt{R} package \cite[]{ni_bayesian_2019}.
