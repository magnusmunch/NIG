...
@
  ...
  of drug and feature variance components is due to near-unidentifiability. 
  In any case, the consequences are limited, since the variances on the 
  $\beta_{jd}$ level are left unbiased
...
<<simulations_gdsc_kurt, fig.cap="Simulation results for Scenario 3: mean estimated prior Kurtoses $\\mathcal{K}(\\beta_{jd})$ versus true values, with line of identity (dotted).", out.width="50%", fig.asp=1>>= 
@
...
colnames(tab5.2) <- c("lasso$_{\\lambda^{\\text{CV}}}$($\\sim$50)$^*$", 
                      "lasso(25)", "lasso(100)", 
...
@
  ...
  \begin{itemize}
	  \item{Analysis 1 results in a positive additive effect of the mutation 
	    ...
	    (Table \ref{tab:analysis_gdsc_fit1}). 
	    This translates to more $\bm{\beta}_d$ shrinkage towards zero with 
	    increasing $p$-value, as expected.}
	  ...
	  \item{Analysis 3 indicates that CCLE $p$-values are positively related
	    to prior precision (Table \ref{tab:analysis_gdsc_fit3}), i.e., higher 
	    CCLE $p$-values results in 
...
@
  ...
  In general however, all models perform very similarly. The ranges of mean PMSE 
...
kableExtra::add_footnote(kableExtra::kable_styling(knitr::kable(
  ...
  format="latex", booktabs=TRUE, escape=FALSE), 
  latex_options=c("HOLD_position")), 
  label="lasso with cross validated $\\lambda$ selects, on average, 42-56 features in all analyses (indicated with $\\sim$50).",
...
@
  ...
  \cite[]{zeng_incorporating_2020}. Here, bSEM is designed to include
  only dichotomous external covariates, so is not applicable in most of the
  applications and simulations that we have considered here. Inclusion of
  multiple external features is allowed by \texttt{xtune}, but not on
  the drug level, so \texttt{xtune} has limited applicability in our simulations 
  and applications.
  ...
  hyperparameters.
	...
	such an external
	covariate for the genes is gene length, where we suspect that longer genes are
