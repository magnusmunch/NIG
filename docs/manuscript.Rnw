...
@
  ...
  \url{https://github.com/magnusmunch/NIG}.
	...
	\ref{sm-sec:mxtune}. Additionally, SM Section \ref{sm-sec:simulations} 
...
# simulation 2
res <- read.table("results/simulations_gdsc_res2.2.txt", row.names=NULL)
temp <- res[, 1]
res <- as.matrix(res[, -1])
rownames(res) <- temp
...
# simulation 3
res <- read.table("results/simulations_gdsc_res3.2.txt", row.names=NULL)
temp <- res[, 1]
res <- as.matrix(res[, -1])
rownames(res) <- temp
...
tab5.2 <- tab5[, c(4, 13, 14, 6:8, 10:12)]
tabm5.2 <- tabm5[, c(4, 13, 14, 6:8, 10:12)]
...
colnames(tab5.2) <- c("lasso", "lasso(25)", "lasso(100)", 
                      "NIG$_{\\text{f}+\\text{d}}+$DSS(lasso)",
...
@
  ...
  as lasso, 25, or 50 features. Table 
