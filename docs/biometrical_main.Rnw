...
rotating,natbib,titlesec,multicol,hyperref,verbatim,
bbm,algorithm,algpseudocode,pgfplotstable,threeparttable,booktabs,mathtools,
dsfont,parskip,xr,tikz,times,w-thm}
...
\begin{document}
  ...
  \DOIsuffix{bimj.200100000}\Volume{XX}\Issue{XX}\Year{XXX}
  \pagespan{1}{}
  \keywords{Drug sensitivity; Empirical Bayes; 
  Genomics of Drug Sensitivity in Cancer (GDSC); Variational Bayes\\[1pc]
  \noindent\hspace*{-4.2pc} Supporting Information for this article is available
  from the author or on the WWW under\break \hspace*{-4pc} 
  \underline{http://dx.doi.org/10.1022/bimj.XXXXXXX} (please delete if not
  applicable)}  %%% semicolon and fullpoint added here for keyword style

  \title[Drug sensitivity prediction with the NIG prior]
  {Drug sensitivity prediction with the normal inverse Gaussian prior 
  informed by external data}

  %% Institutes
  \address[\inst{1}]{Department of Epidemiology \& Biostatistics, 
  Amsterdam UMC, VU University, PO Box 7057, 1007 MB Amsterdam, The Netherlands}
  \address[\inst{2}]{Mathematical Institute, Leiden University, Leiden, 
  The Netherlands}
  \address[\inst{3}]{MRC Biostatistics Unit, 
  Cambridge Institute of Public Health, Cambridge, United Kingdom}

  %% Authors
  \author[M\"unch]{Magnus M. M\"unch
  \footnote{Corresponding author: {\sf{e-mail: m.munch@amsterdamumc.nl}}, 
  Phone: +31 71 527 4844}\inst{,1,2,3}} 
  \author[van de Wiel]{Mark A. van de Wiel\inst{1,3}}
  \author[Richardson]{Sylvia Richardson\inst{3}}
  \author[Leday]{Gwena{\"e}l G. R. Leday\inst{3}}
...
@
  ...
  \url{https://github.com/magnusmunch/cambridge}.
	...
	covariates in the estimation called NIG1 in the following, (ii) the NIG model
	estimated as in Section \ref{sec:estimation} called NIG2, and regular (iii)
	lasso and (iv) ridge models.
	Exclusion of the external covariates as in the NIG1 model
	amounts to direct estimation of one common expected 
	prior means $\phi$ and/or $\chi$, instead of regression estimates for the
	$\phi_{jd}$ and/or $\chi_d$ as in the NIG2 model. All 
	models are fit on 241 randomly selected cell lines, while performance measures 
	are estimated from the remaining 241 cell lines. We repeat the whole
	simulation 50 times.
...
tab <- cbind(apply(res[rownames(res)=="elbo", ], 2, median),
             apply(res[rownames(res)=="elbot", ], 2, median) -
               apply(res[rownames(res)=="elbo", ], 2, median),
             apply(res[rownames(res)=="emse", ], 2, median),
             apply(res[rownames(res)=="pmse", ], 2, median),
             apply(res[rownames(res)=="pmse", ], 2, median) - 
               apply(res[rownames(res)=="pmset", ], 2, median))
tab <- round(tab, 3)
...
tab[which(tab[, 5]==min(tab[, 5], na.rm=TRUE)), 5] <- 
  paste0("\\textbf{", tab[which(tab[, 5]==min(tab[, 5], na.rm=TRUE)), 5], "}")
colnames(tab) <- c("$\\text{ELBO}_{\\text{test}}$",
                   "$\\text{ELBO}_{\\text{train}} 
                   - \\text{ELBO}_{\\text{test}}$",
                   "$\\text{EMSE}_{\\text{test}}$",
                   "$\\text{PMSE}_{\\text{test}}$",
                   "$\\text{PMSE}_{\\text{test}} 
                   - \\text{PMSE}_{\\text{train}}$")     
@	
  Figure \ref{fig:simulations_gdsc_est1} shows the estimated
	$\bm{\alpha}_{\text{feat}}$ together with its true value for NIG2 in 
	version 1 of the simulation study. Figure 
...
<<simulations_gdsc_est1, fig.cap="Estimated (a) hyperparameters $\\bm{\\alpha}_{\\text{feat}}$ and (b) prior means $\\bm{\\phi}_{jd}$ from simulation version 1 with fixed $\\tau_d^2$.", out.width="100%", fig.asp=1/2>>= 
@
	
	Model NIG1 gives a median $\phi$ estimate of \Sexpr{round(est.phi, 2)},  
	...
	Table \ref{tab:simulations_gdsc_tab1} displays several median performance
	measures for simulation version 1. The table includes as a reference 
	the null model, which
	is an empty model that only fits the means of the $\y_{d}$.
	The median evidence lower bounds (ELBO) approximates the marginal
	likelihoods of models NIG1 and NIG2 on both training and test data, while 
	estimation MSE (EMSE) and prediction MSE (PMSE) give measures of predictive 
	performance. Focussing on MSE, we see that the NIG2 model, that includes
	the external covariates, performs best, both in terms of estimation and 
	prediction. Model fit is best for the NIG1 model that does not include the
	external covariates, according to the ELBO, calculated on the test data.
	The lower MSEs confirm that the model that
	includes the external data (NIG2) learns the underlying structure 
	in the data better than model NIG1 that does not include the external 
	data.
...
kable(tab, col.names=colnames(tab), align="r",
      caption="Median performance measure estimates for simulation version 1 (best performing model in bold).", 
...
@	
  Figure \ref{fig:simulations_gdsc_est2} shows the estimated
	$\bm{\alpha}_{\text{drug}}$ together with its true value for NIG2 in 
	version 2 of the simulation study. They are estimated without bias.
	Median $\lambda_{\text{drug}}=\Sexpr{round(est.lambdad[2], 2)}$ is slightly 
	overestimated compared to true value 1. 
	The median $\chi$ estimate in the NIG1 model is \Sexpr{round(est.chi, 2)},  
	which is almost equal to the true mean \Sexpr{round(true.chi, 2)}. 
	$\lambda_{\text{drug}}$ is underestimated with median estimate
	\Sexpr{round(est.lambdad[1], 2)} compared to the true value.
	Median performance measures for simulation version 2 are displayed in Table
	\ref{tab:simulations_gdsc_tab2}. Again, we see that NIG2 performs best in 
	terms of MSE. However, NIG1 performance is the same here, indicating that 
	the external covariates do not add much here.
	Model fit is again best for the NIG1 model.
<<simulations_gdsc_est2, fig.cap="Estimated (a) hyperparameters $\\bm{\\alpha}_{\\text{feat}}$ and (b) prior means $\\bm{\\phi}_{jd}$ from simulation version 2 with fixed $\\gamma_{jd}^2$.", out.width="100%", fig.asp=1/2>>= 
@
...
options(knitr.kable.NA="", digits=3)
kable(tab, col.names=colnames(tab), align="r",
      caption="Median performance measure estimates for simulation version 2 (best performing model in bold).", 
      format="latex", booktabs=TRUE, escape=FALSE) %>%
  kable_styling(latex_options=c("HOLD_position"))
@
...
phi.ratio <- apply(apply(sapply(0:3, function(s) {
  res[rownames(res)==paste0("alphaf" ,s), colnames(res)=="NIG2"]}) %*% 
    t(cbind(1, rbind(0, diag(3)))), 2, function(m) {
      res[rownames(res)=="alphaf0", colnames(res)=="NIG2"]/m}), 2, median)
chi.ratio <- apply(apply(sapply(0:3, function(s) {
  res[rownames(res)==paste0("alphad" ,s), colnames(res)=="NIG2"]}) %*% 
    t(cbind(1, rbind(0, diag(3)))), 2, function(m) {
      res[rownames(res)=="alphad0", colnames(res)=="NIG2"]/m}), 2, median)
...
@	
  In Figure \ref{fig:simulations_gdsc_est3} the 
	$\bm{\alpha}_{\text{feat}}$ and $\bm{\alpha}_{\text{drug}}$ estimated by NIG2
	are displayed together with their true values for
	simulation 3. $\bm{\alpha}_{\text{feat}}$ are underestimated, while 
	...
	value 1. The NIG1 model is also consistently over- and underestimating $\phi$
	and $\chi$ with estimates \Sexpr{round(est.phi, 2)} and 
	\Sexpr{round(est.chi, 2)}, respectively (compared to the true mean 
	\Sexpr{round(true.chi, 2)}). NIG1 also over- and underestimates 
	...
	that NIG2 outperforms the other models in terms of prediction and estimation
	MSE.
<<simulations_gdsc_est3, fig.cap="Estimated (a) hyperparameters $\\bm{\\alpha}_{\\text{feat}}$ and (b) prior means $\\bm{\\phi}_{jd}$ from simulation version 3.", out.width="100%", fig.asp=1>>= 
@
...
options(knitr.kable.NA="", digits=3)
kable(tab, col.names=colnames(tab), align="r",
      caption="Median performance measure estimates for simulation version 3 (best performing model in bold).", 
      format="latex", booktabs=TRUE, escape=FALSE) %>%
  kable_styling(latex_options=c("HOLD_position"))
@
	...
	and number of features and samples in the thousands are computationally 
	feasible.
	...
	variables, named NIG1 and NIG2 in the following. NIG1 does 
	not use the external covariates and estimates
	one overall $\phi$ and one overall $\chi$, while NIG2 uses the external 
	covariates to estimate both $\bm{\alpha}_{\text{feat}}$ and
	$\bm{\alpha}_{\text{drug}}$, while NIG3 and NIG4 estimate only 
	$\bm{\alpha}_{\text{feat}}$ or $\bm{\alpha}_{\text{drug}}$, respectively,
	...
	$\gamma_{jd}^2 \sim \mathcal{IG} (\phi_{jd}, \lambda_{jd})$. Additionally,
	the bSEM model can
...
tabm <- tab
tabm <- round(tabm, 3)
tabm[which(tabm[, 1]==max(tabm[, 1], na.rm=TRUE)), 1] <- 
  paste0("\\textbf{", tabm[which(tabm[, 1]==max(tabm[, 1], na.rm=TRUE)), 1], "}")
tabm[which(tabm[, 2]==min(tabm[, 2], na.rm=TRUE)), 2] <- 
  paste0("\\textbf{", tabm[which(tabm[, 2]==min(tabm[, 2], na.rm=TRUE)), 2], "}")
tabm[which(tabm[, 3]==min(tabm[, 3], na.rm=TRUE)), 3] <- 
  paste0("\\textbf{", tabm[which(tabm[, 3]==min(tabm[, 3], na.rm=TRUE)), 3], "}")
colnames(tabm) <- c("$\\text{ELBO}_{\\text{test}}$",
                   "$\\text{ELBO}_{\\text{train}} 
                   - \\text{ELBO}_{\\text{test}}$",
                   "$\\text{PMSE}_{\\text{test}}$")  
...
@
  ...
  external covariates. Models NIG2, NIG3, and bSEM estimate a negative effect 
  for
  ...
  are not in the target pathway, according to expectation. Furthermore the NIG2
  ...
  prior precisions for the regression parameters for approved drugs, i.e.,
  approved drugs are easier to predict than experimental and developmental 
  drugs. On the other hand, the NIG4 model, that only includes external
  drug covariates, estimates a negative effect for the developmental drugs.
  The NIG2 model estimates the smallest prior $\bm{\beta}_d$ precision for the
  ...
  to expectation. Somewhat surprisingly, the NIG4 model estimates the smallest
  ...
  NIG2 and bSEM, respectively. This confirms that the estimated prior 
...
@
  ...
  (PMSE of \Sexpr{round(tab[6, 3], 3)}), while bSEM 
  (PMSE of \Sexpr{round(tab[7, 3], 3)}) performs worse than 
  the null model (PMSE of \Sexpr{round(tab[8, 3], 3)}). Second and third best
  performing models are NIG1 and NIG2 (PMSEs of \Sexpr{round(tab[1, 3], 3)} and
  \Sexpr{round(tab[2, 3], 3)}, respectively), both outperforming the lasso
  \Sexpr{round(tab[5, 3], 3)}. Part of the differences in performance between
  ...
  \Sexpr{round(kurts[2], 3)}, and \Sexpr{round(kurts[3], 3)} for the NIG2, 
  ...
  means that the NIG uses less of the $\bm{\beta}_d$ to predict.
<<analysis_gdsc_tab1>>=
kable(tabm,
...
@	
	...
	covariates (NIG2 in Section \ref{sec:gdsc}) on the GDSC data from Section
	...
	reasonably able to recover hyperparameters. In addition, the simulations
	show that inclusion of informative external covariates is indeed beneficial to
	predictive performance. The model is put into practice on the GDSC data. 
	The results in Section
	\ref{sec:gdsc} show that the inclusion of the external covariates is 
	beneficial for drug sensitivity prediction. Prediction MSE of the NIG
	model with both both feature and drug variance components is lower
	than for the regular lasso model and although the prediction MSE is
	not lower if external covariates are included, the resulting hyperparameter 
	estimates are still informative and may guide future research efforts.
	In addition, the comparison to the bSEM model that allows for the inclusion of 
	...
	compared to bSEM. Consequently, the total prediction MSE is significantly
	lower for the NIG model. 

	\section*{Supplementary Material}
	Source code to reproduce the manuscript and the contained results is available
	as Supporting Information on the journal’s web page 
	(http://onlinelibrary.wiley.com/doi/xxx/ suppinfo) and from
	\url{https://github.com/magnusmunch/cambridge}. 
	
	\section*{Acknowledgements}
	A visit of MM to SR and GL was funded by the Amsterdam Public Health 
	institute's Methodology travel grant.
