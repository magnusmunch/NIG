...
\usepackage{bm,amsmath,amssymb,amsthm,amsfonts,graphics,epsfig,
rotating,appendix,titlesec,multicol,hyperref,verbatim,
...
\begin{document}
  ...
  \title[Drug sensitivity prediction with NIG prior]
  ...
  \Receiveddate{zzz} \Reviseddate{zzz} \Accepteddate{zzz} 
	...
	repositories might provide extra information such as the molecular weight of
	the compounds or the publication signatures of the molecular features. In some
	cases, prior knowledge on the drug efficacies might be available,
	...
	inverse Gaussian model, where the amount of regularisation is estimated by an 
  ...
  and features in a data-driven manner.
	...
	GDSC data, and we end with a discussion in Section \ref{sec:discussion}.
	...
	prior 
	variance into a local components $\gamma_{jd}^2$ and a global component
	...
	capture the general trend in $\beta_{jd}$. Addionally, for appropriate local
	shrinkage in global-local 
  ...
  $\tau_d^2=\sigma_d^2=1$, the resulting
...
@  
  ...
  prior.
	...
	(binary) features covariate is target pathway presence, with $c_{jdg}=0$ if 
  ...
  \url{https://github.com/magnusmunch/cambridge}.
	...
	\section{Simulations}
	\subsection{Fixed drug component}
	...
	A pre-processing step, as explained in Section \ref{sec:gdsc}, selects around 
	300 features per drug, while the 168 drug sensitivities for 482 cell lines are 
	simulated from model (\ref{eq:likelihood}) and 
	(\ref{eq:prior}), with two adjustments: we fix $\forall d: \tau_d^2=1$ 
	and draw the error variances as $\forall d: \sigma_d^2 \sim \Gamma^{-1}(3,2)$,
	such that the prior $\sigma_d^2$ mean and variance are both one.
	We create four external dummy feature covariates that code for four 
	randomly assigned groups of features. We set $\bm{\alpha}_{\text{feat}}$ such
	...
	The prior scale parameter is set to $\lambda_{\text{feat}}=1$. We estimate 
	four models: (i) the NIG model as in Section \ref{sec:estimation}, (ii) an 
	NIG model where we fix $\lambda_{\text{feat}}$ to its true value, and two NIG
	models without the inclusion of the external covariates, again with 
	$\lambda_{\text{feat}}$ either (iii) estimated or (iv) fixed to its true 
	value. Exclusion of the external covariates as in models (iii) and (iv) 
	amounts to direct estimation of one common expected 
	prior mean $\phi$, instead of regression estimates for the $\phi_{jd}$ as
	in models (i) and (ii). All 
...
mapmse <- apply(res[, c(7:12)], 2, median)
mapmset <- apply(res[, c(13:18)], 2, median)
melbo <- apply(elbo, 2, median)
phi <- 1/(set$alphaf %*% t(unname(model.matrix(~ factor(1:4)))))
mphi <- apply(1/est[, c(11, 13)], 2, median)
mlambdaf <- apply(est[, c(5, 12)], 2, median)
@	
	Figure \ref{fig:simulations_gdsc_est1} shows the estimated
	$\bm{\alpha}_{\text{feat}}$ together with its true value for models (i) and 
	(ii). The figure shows that $\bm{\alpha}_{\text{feat}}$ is consistently
	underestimated 
	in the full NIG model (both with estimated and fixed $\lambda_{\text{feat}}$),
	while $\lambda_{\text{feat}}$ is overestimated as 
	\Sexpr{round(mlambdaf[1], 2)} in model (i) (the true value is 1). 
	This translates into
	underestimated prior means $\phi_{jd}$ as presented in Figure 
	\ref{fig:simulations_gdsc_phi1}, which in turn implies underestimated 
	$\beta_{jd}$ variances. The overestimated $\lambda_{\text{feat}}$ translates 
	...
	$\bm{\alpha}_{\text{feat}}$ and $\lambda_{\text{feat}}$ are higher level 
	parameters, which are, in general, hard to estimate. We note 
	that even though $\bm{\alpha}_{\text{feat}}$ and thus $\phi_{jd}$ are 
	estimated with bias, the orders of their elements are correct.
<<simulations_gdsc_est1, fig.cap="Estimated hyperparameters $\\bm{\\alpha}_{\\text{feat}}$ from simulations with (a) estimated and (b) fixed $\\lambda_{\\text{feat}}$ (models (i) and (ii), respectively).", out.width="100%", fig.asp=1/2>>= 
@
	
<<simulations_gdsc_phi1, fig.cap="Estimated prior means $\\bm{\\phi}_{jd}$ for the four groups of features from simulations with (a) estimated and (b) fixed $\\lambda_{\\text{feat}}$ (models (i) and (ii), respectively).", out.width="100%", fig.asp=1/2>>= 
@
	
	The models (iii) and (iv) give median $\phi$ estimates of
	\Sexpr{round(mphi[1], 2)} and \Sexpr{round(mphi[2], 2)}, 
	respectively. The $\phi$ estimates are
	underestimated, compared to true $\phi$ mean \Sexpr{round(mean(phi), 2)}. 
	Model (iii) slightly underestimates the median
	$\lambda_{\text{feat}}$ at \Sexpr{round(mlambdaf, 2)[2]} 
	(the true value is one) and thus overestimates the 
	prior $\gamma_{jd}^2$ variance. This is likely to compensate for the common
	prior mean 
	$\phi$, that is not able to capture part of the variation in the true feature
	specific $\phi_{jd}$. The median evidence 
	...
	\ref{tab:pmse1}. Table \ref{tab:pmse1} confirms that the models that
...
mapmse <- apply(res[, c(7:12)], 2, median)
mapmset <- apply(res[, c(13:18)], 2, median)
tab <- rbind(mapmse, mapmse - mapmset)[, -c(5, 6)]
rownames(tab) <- c("$\\text{MSE}_{\\text{test}}$",
                  "$\\text{MSE}_{\\text{test}} - \\text{MSE}_{\\text{train}}$")
options(knitr.kable.NA="", digits=3)
kable(tab,
      col.names=c("(i)", "(ii)", "(iii)", "(iv)"),
      caption="Median prediction MSEs for the models (i)-(iv) in the fixed 
      $\\tau_d^2$ simulation setting.", 
      format="latex", booktabs=TRUE, escape=FALSE) %>%
  kable_styling(latex_options=c("HOLD_position"))
@
	
  \subsection{Random drug component}
...
melbo <- apply(elbo, 2, median)
phi <- 1/(set$alphaf %*% t(unname(model.matrix(~ factor(1:4)))))
chi <- 1/(set$alphad %*% t(unname(model.matrix(~ factor(1:4)))))
mphi <- apply(1/est[, c(21, 25)], 2, median)
mchi <- apply(1/est[, c(22, 26)], 2, median)
mlambdaf <- apply(est[, c(9, 23)], 2, median)
mlambdad <- apply(est[, c(10, 24)], 2, median)
@	
	Figures \ref{fig:simulations_gdsc_est2} and 
	\ref{fig:simulations_gdsc_phi2} display the estimated $\bm{\alpha}$, 
	$\phi_{jd}$, and $\chi_d$ for models (i) and (ii). The scale parameters were
	estimated as $\lambda_{\text{feat}}=\Sexpr{round(mlambdaf[1], 2)}$ and
	$\lambda_{\text{drug}}=\Sexpr{round(mlambdad[1], 2)}$ in model (i). From 
...
<<simulations_gdsc_est2, fig.cap="Estimated hyperparameters $\\bm{\\alpha}_{\\text{feat}}$ and $\\bm{\\alpha}_{\\text{drug}}$ from simulations with (a) \\& (c) estimated and (b) \\& (d) fixed $\\lambda_{\\text{feat}}$ and $\\lambda_{\\text{drug}}$ (models (i) and (ii), respectively).", out.width="100%", fig.asp=1>>= 
@
	
<<simulations_gdsc_phi2, fig.cap="Estimated prior means $\\bm{\\phi}_{jd}$ and $\\bm{\\chi}_{d}$ for the four groups of features and drugs from simulations with (a) \\& (c) estimated and (b) \\& (d) fixed $\\lambda_{\\text{feat}}$ and $\\lambda_{\\text{drug}}$ (models (i) and (ii), respectively).", out.width="100%", fig.asp=1>>= 
@
...
kable(tab,
      col.names=c("(i)", "(ii)", "(iii)", "(iv)"),
      caption="Median prediction MSEs for the models (i)-(iv) in the random 
      $\\tau_d^2$ simulation setting.", 
      format="latex", booktabs=TRUE, escape=FALSE) %>%
  kable_styling(latex_options=c("HOLD_position"))
@
...
@
  ...
  (\Sexpr{round(mapmse[2], 2)} and \Sexpr{round(mapmse[3], 2)}, in the
  ...
  \Sexpr{round(mapmse[4], 2)} compared to the null model's MSE of 
  \Sexpr{round(mapmse[1], 2)}.
<<results_data>>= 
load("results/data_gdsc_res1.Rdata")
