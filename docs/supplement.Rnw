...
<<"lines_igaussian_res1_elbo", fig.cap="Convergence of ELBO for a) inverse Gaussian, b) independent inverse Gaussian, and c) inverse Gamma models", fig.align="center", echo=FALSE>>=
...
<<"lines_igaussian_res1_theta", fig.cap="Convergence of prior means for a) inverse Gaussian, b) independent inverse Gaussian, and c) inverse Gamma models", fig.align="center", echo=FALSE>>=
fit1 <- read.table("../results/simulations_igaussian_fit1.csv")
par(cex=1.3)
layout(matrix(c(rep(c(1, 1, 2, 2), times=2), rep(c(0, 3, 3, 0), times=2)), 
              4, 4, byrow=TRUE), widths=1, heights=1, respect=TRUE)
...
<<"boxplot_igaussian_res2_theta", fig.cap="a) MSE and b) correlation for prior means of $\\gamma_d^2$ ($\\theta_d$) in simulation 2", fig.align="center", echo=FALSE>>=
...
temp3 <- list(apply(t(temp1[[1]]), 2, function(r) {
  cor(r, as.numeric(set2[, substr(colnames(set2), 1, 5)=="theta"]))}),
              apply(t(temp1[[2]]), 2, function(r) {
                cor(r, as.numeric(set2[, substr(colnames(set2), 1, 5)=="theta"]))}),
              apply(t(temp1[[4]]/(temp1[[3]] - 1)), 2, function(r) {
                cor(r, unique(as.numeric(set2[, substr(colnames(set2), 1, 5)=="theta"])))}))
par(cex=1.3)
layout(matrix(c(rep(1, 4), rep(2, 4)), 2, 4), widths=1, heights=1, respect=TRUE)
boxplot(temp2, names=names1, ylab="MSE", main="a)")
legend("bottomright", title=expression(bold("median MSE")), bty="n",
...
names1 <- c("inv Gauss", "ind inv Gauss")
temp1 <- list(res2[, substr(colnames(res2), 1, 15)=="alpha.inv.Gauss"],
              res2[, substr(colnames(res2), 1, 19)=="alpha.ind.inv.Gauss"])
temp2 <- list(colMeans((t(temp1[[1]]) - as.numeric(set2[, substr(colnames(set2), 1, 5)=="alpha"]))^2),
              colMeans((t(temp1[[2]]) - as.numeric(set2[, substr(colnames(set2), 1, 5)=="alpha"]))^2))
temp3 <- list(apply(t(temp1[[1]]), 2, function(r) {cor(r, as.numeric(set2[, substr(colnames(set2), 1, 5)=="alpha"]))}),
              apply(t(temp1[[2]]), 2, function(r) {cor(r, as.numeric(set2[, substr(colnames(set2), 1, 5)=="alpha"]))}))
par(cex=1.3)
layout(matrix(c(rep(1, 4), rep(2, 4)), 2, 4), widths=1, heights=1, respect=TRUE)
boxplot(temp2, names=names1, ylab="MSE", main="a)")
legend("topright", title=expression(bold("median MSE")), bty="n",
...
for(i in 2:5) {
  lines(fit2[, substr(colnames(fit2), 1, 15)=="inv.Gauss.alpha"][, 2], col=i)  
}
@
