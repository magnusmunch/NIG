...
temp2 <- cbind(colMeans((2*temp1[[1]]/(set1[rownames(set1)=="n", ] + set1[
  rownames(set1)=="p", ] - 1) - set1[substr(rownames(set1), 1, 5)=="sigma", ]^2)^2),
  colMeans((2*temp1[[2]]/(set1[rownames(set1)=="n", ] - 1) - 
              set1[substr(rownames(set1), 1, 5)=="sigma", ]^2)^2),
...
temp2 <- list(as.matrix(sqrt(temp1[[1]]*temp1[[2]]^2/temp1[[3]]))*
                ratio_besselK_cpp(as.matrix(sqrt(
                  temp1[[1]]*temp1[[3]])/temp1[[2]]), set1[rownames(set1)=="p", ]),
              as.matrix(sqrt(temp1[[4]]*temp1[[5]]^2/temp1[[6]]))*
                ratio_besselK_cpp(as.matrix(sqrt(
                  temp1[[4]]*temp1[[6]])/temp1[[5]]), set1[rownames(set1)=="p", ]),
              as.matrix(temp1[[8]]/(temp1[[7]] - 1)))
temp3 <- sapply(temp2, function(m) {
  colMeans((t(m) - set1[substr(rownames(set1), 1, 5)=="gamma", ]^2)^2)})
temp4 <- sapply(temp2, function(m) {apply(t(m), 2, function(r) {
  cor(r, set1[substr(rownames(set1), 1, 5)=="gamma", ]^2)})})
par(cex=1.3)
...
temp2 <- list(colMeans((t(temp1[[1]]) - 
                          set1[substr(rownames(set1), 1, 5)=="theta", ])^2),
              colMeans((t(temp1[[2]]) - 
                          set1[substr(rownames(set1), 1, 5)=="theta", ])^2),
              colMeans((t(temp1[[4]]/(temp1[[3]] - 1)) - 
                          unique(set1[substr(rownames(set1), 1, 5)=="theta", ]))^2))
temp3 <- list(apply(t(temp1[[1]]), 2, function(r) {
  cor(r, set1[substr(rownames(set1), 1, 5)=="theta", ])}),
              apply(t(temp1[[2]]), 2, function(r) {
                cor(r, set1[substr(rownames(set1), 1, 5)=="theta", ])}),
              apply(t(temp1[[4]]/(temp1[[3]] - 1)), 2, function(r) {
                cor(r, unique(set1[substr(rownames(set1), 1, 5)=="theta", ]))}))
par(cex=1.3)
layout(matrix(c(rep(1, 4), rep(2, 4)), 2, 4), widths=1, heights=1, respect=TRUE)
...
res1 <- read.table("../results/simulations_igaussian_res1.csv")
set1 <- read.table("../results/simulations_igaussian_set1.csv")
set1[substr(rownames(set1), 1, 5)=="alpha", ]
...
temp2 <- list(colMeans((t(temp1[[1]]) - set1[substr(rownames(set1), 1, 5)=="alpha", ])^2),
              colMeans((t(temp1[[2]]) - set1[substr(rownames(set1), 1, 5)=="alpha", ])^2))
temp3 <- list(apply(t(temp1[[1]]), 2, function(r) {cor(r, set1[substr(rownames(set1), 1, 5)=="alpha", ])}),
              apply(t(temp1[[2]]), 2, function(r) {cor(r, set1[substr(rownames(set1), 1, 5)=="alpha", ])}))
par(cex=1.3)
layout(matrix(c(rep(1, 4), rep(2, 4)), 2, 4), widths=1, heights=1, respect=TRUE)
boxplot(temp2, names=names1, ylab="MSE", main="a)")
legend("topright", title=expression(bold("median MSE")), bty="n",
