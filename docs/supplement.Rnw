...
<<figures, cache=FALSE, echo=FALSE>>=
...
\begin{document}
  ...
  \delta_d^{(h+1)} & = \frac{n+p+1}{2 \zeta_d^{(h)}} [\trace(\bm{\Sigma}_d^{(h+1)}) + (\bm{\mu}_d^{(h+1)}) \tr \bm{\mu}_d^{(h+1)}] + \lambda, \\ \zeta_d^{(h+1)} & = \frac{1}{2} \bigg[\mathbf{y}_d \tr \mathbf{y}_d -2 \mathbf{y}_d \tr \X \bm{\mu}_d^{(h+1)} + \trace ( \X \tr \X \bm{\Sigma}_d^{(h+1)}) + (\bm{\mu}_d^{(h+1)}) \tr \X \tr \X \bm{\mu}_d^{(h+1)} + \\
  ...
  \begin{align*}
  q(\bm{\beta}) & \overset{D}{=} \prod_{d=1}^D \mathcal{N}_p (\bm{\mu}_d, \bm{\Sigma}_d), \\
  q(\bm{\gamma}^2) & \overset{D}{=} \prod_{d=1}^D \Gamma^{-1} \( \frac{p}{2} + a_{\text{class}(d)}, b^*_{\text{class}(d)}\), \\
  q(\bm{\sigma}^2) & \overset{D}{=} \prod_{d=1}^D \Gamma^{-1} \( \frac{n+p}{2} + 0.001, d^*_{\text{class}(d)} \),
  \end{align*}
  with variational parameter updates:
  \begin{align*}
  \bm{\Sigma}_d^{(h+1)} & = \frac{n + p - 1.998}{2(d^*_{\text{class}(d)})^{(h)}}\(\X \tr \X + \frac{p + 2a_{\text{class}(d)} - 2}{2(b^*_{\text{class}(d)})^{(h)}} \I_p\)^{-1}, \\
  \bm{\mu}_d^{(h+1)} & = \(\X \tr \X + \frac{p + 2a_{\text{class}(d)} - 2}{2(b^*_{\text{class}(d)})^{(h)}} \I_p\)^{-1} \X \tr \y_d, \\
  b^*_{\text{class}(d)} & = b_{\text{class}(d)} + \frac{(d^*_{\text{class}(d)})^{(h)}}{n + p -1.998} [\trace(\bm{\Sigma}_d^{(h+1)}) + (\bm{\mu}_d^{(h+1)}) \tr \bm{\mu}_d^{(h+1)}],\\
  d^*_{\text{class}(d)} & = \frac{1}{2} \[\mathbf{y}_d \tr \mathbf{y}_d -2 \mathbf{y}_d \tr \X \bm{\mu}_d^{(h+1)} + \trace (\X \tr \X \bm{\Sigma}_d^{(h+1)}) + (\bm{\mu}_d^{(h+1)}) \tr \X \tr \X \bm{\mu}_d^{(h+1)}\].
  \end{align*}
