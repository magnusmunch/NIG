...
@
  ...
  With this interval it is straightforward to find $\alpha^{(l+1)}_c$ by any root-finding algorithm and plugging the solution into (\ref{eq:estequation2}) to find $\lambda^{(l+1)}_c$.
	...
	We monitor convergence through the evidence lower bound (ELBO). To compute the evidence lower bound after updating the EB parameters from $\alpha_d^{(l)}, \theta_d^{(l)}, \lambda_d^{(l)}$ to $\alpha_d^{(l + 1)}, \theta_d^{(l + 1)}, \lambda_d^{(l + 1)}$, we compute $\text{ELBO}^{(l + 1)} = \sum_{d=1}^D \text{ELBO}_d^{(l + 1)}$, with, in the inverse Gaussian conjugate case:
	\begin{align*}
	...
	& - \frac{df + 1}{2} \log \zeta_d^{(l)} + \frac{b_d^{(l)}}{2} \[ \delta_d^{(l)} - a_d^{(l)} (\bm{\mu}_d^{(l)}) \tr \bm{\mu}_d^{(l)} - a_d^{(l)} \trace (\bm{\Sigma}_d^{(l)}) \] + \frac{p + 1}{4} \log \lambda_d^{(l)} \\
	& - \frac{p + 1}{2} \log \theta_d^{(l)} - \frac{p+1}{4} \log \delta_d^{(l)} + \log K_{\frac{p+1}{2}}\(\sqrt{\lambda_d^{(l)} \delta_d^{(l)}}/\theta_d^{(l)}\) \\
	& + \frac{\lambda_d^{(l)} e_d^{(l)}}{2(\theta_d^{(l)})^2} +\frac{\lambda_d^{(l+1)}}{\theta_d^{(l+1)}} + \frac{1}{2}\log \lambda_d^{(l+1)} - \frac{\lambda_d^{(l+1)} e_d^{(l)}}{2 (\theta_d^{(l + 1)})^2} - \frac{\lambda_d^{(l + 1)} b_d^{(l)}}{2}.
	\end{align*}
	
	In the conjugate inverse Gamma setting, we have:
	\begin{align*}
	\text{ELBO}_d^{(l + 1)} & = \frac{1}{2} \log |\bm{\Sigma}_d^{(l)}| - \frac{a_d^{(l)}}{2} \[ \y_d \tr \y_d - 2\y_d \tr \X \bm{\mu}_d^{(l)} + (\bm{\mu}_d^{(l)}) \tr \X \tr \X \bm{\mu}_d^{(l)} + \trace ( \X \tr \X \bm{\Sigma}_d^{(l)})\] \\
	& - \frac{df + 1}{2} \log \zeta_d^{(l)} + \frac{b_d^{(l)}}{2} \[ \delta_d^{(l)} - a_d^{(l)} (\bm{\mu}_d^{(l)}) \tr \bm{\mu}_d^{(l)} - a_d^{(l)} \trace (\bm{\Sigma}_d^{(l)}) \] -\frac{\alpha_d^{(l)}}{2} \psi \(\frac{p + \alpha_d^{(l)}}{2}\) \\
	& + \log \Gamma \( \frac{p + \alpha_d^{(l)}}{2} \) - \frac{p+\alpha_d^{(l + 1)}}{2} \log \delta_d^{(l)} - \log \Gamma \( \frac{\alpha_d^{(l + 1)}}{2} \) + \frac{\alpha_d^{(l + 1)}}{2} \psi \(\frac{p + \alpha_d^{(l)}}{2}\) \\
	& + \frac{\alpha_d^{(l + 1)}}{2} \log \lambda_d^{(l+1)} - \frac{\lambda_d^{(l + 1)} b_d^{(l)}}{2}.
	\end{align*}
	
	The non-conjugate ELBO versions of both the inverse Gaussian and inverse Gamma models simply exchanges the $a_d^{(l)} (\bm{\mu}_d^{(l)}) \tr \bm{\mu}_d^{(l)}$ and $a_d^{(l)} \trace (\bm{\Sigma}_d^{(l)})$ terms with $(\bm{\mu}_d^{(l)}) \tr \bm{\mu}_d^{(l)}$ and $\trace (\bm{\Sigma}_d^{(l)})$, respectively. To compute the ELBO after a VB update we simply let $\alpha_d^{(l)}, \theta_d^{(l)}, \lambda_d^{(l)}=\alpha_d^{(l + 1)}, \theta_d^{(l + 1)}, \lambda_d^{(l + 1)}$.
