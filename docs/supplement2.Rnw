...
\begin{document}
	...
	\section{NIG and Student's \texorpdfstring{$t$}{t} Priors}\label{sec:priors}
	\subsection{Normal inverse Gaussian}\label{sec:nigprior}
	Let $\theta = (\mathbf{c} \tr \bm{\alpha})^{-1}$. Then, 
	for a fixed error variance $\sigma^2$, we have marginal prior variance 
	and kurtosis:
	\begin{align*}
	  \V(\beta | \sigma^2) & = \sigma^2 \theta, \\
	  \mathcal{K}(\beta | \sigma^2) & = \frac{3\theta}{\lambda} + 3.
	\end{align*}
	
	\subsection{Student's \texorpdfstring{$t$}{t} prior}\label{sec:studentstprior}
	For a fixed error variance $\sigma^2$, we have marginal prior variance 
	and kurtosis:
	\begin{align*}
	  \V(\beta | \sigma^2) & = \frac{\sigma^2 \xi}{\eta - 2}, \\
	  \mathcal{K}(\beta | \sigma^2) & = \frac{6}{\eta - 4} + 3.
	\end{align*}
	...
	\begin{align*}
	  ...
	  \frac{1}{2} \sum_{j=1}^p 
	  \E \( \frac{\beta_{jd}^2}{\gamma_{jd}^2 \sigma_d^2} \), \\
	  q_{\bbeta_d} (\bbeta_d) & \overset{D}{=} 
	  \mathcal{N}_p (\bmu_d, \bSigma_d), \\
	  & \text{with } \bSigma_d = \E(\sigma_d^{-2})^{-1} 
	  \{\X \tr \X + \diag [\E(\gamma_{jd}^{-2})] \}^{-1}, \\
	  & \text{and } \bmu_d = \{\X \tr \X + \diag [\E(\gamma_{jd}^{-2})] \}^{-1} 
	  \X \tr \y_d.
	\end{align*}
	The variational posterior for $\gamma_{jd}^2$ is given by:
	\begin{align*}
	  \log q_{\gamma_{jd}^{2}}(\gamma_{jd}^{2}) & \propto \E 
	  [\log \pi (\beta_{jd} | \gamma_{jd}^{2}, \sigma_d^2)] + 
	  \log \pi(\gamma_{jd}^{2}) \\
	  & \propto -\frac{1}{2} \log \gamma_{jd}^{2} - \frac{1}{2} 
	  \E \( \frac{\beta_{jd}^2}{\sigma^{2}_d}\) \gamma_{jd}^{-2} - 
	  \frac{3}{2} \log \gamma_{jd}^{2} - 
	  \frac{\lambda_d \{\gamma_{jd}^2 - 
	  [(\mathbf{c}_{d}^j) \tr \bm{\alpha}]^{-1}\}^2}
	  {2[(\mathbf{c}_{d}^j) \tr \bm{\alpha}]^{-2}} \gamma_{jd}^{-2} \\
	  & \propto \( -1 - 1\) \log \gamma_{jd}^{2} - 
	  \frac{\lambda_d[(\mathbf{c}_{d}^j) \tr \bm{\alpha}]^{2}}{2} \gamma_{jd}^2 - 
	  \frac{1}{2}\[ \lambda_d + \E(\sigma_d^{-2}) \E(\beta_{jd}^2)\] 
	  \gamma_{jd}^{-2} , \\
	  q_{\gamma_{jd}^{2}}(\gamma_{jd}^{2}) & \overset{D}{=} \mathcal{GIG} 
	  \(-1, \lambda_d[(\mathbf{c}_{d}^j) \tr \bm{\alpha}]^{2}, \delta_{jd} \), \\
	  & \text{with } \delta_{jd} =\E(\sigma_d^{-2})
	  \[\E(\beta_{jd})^2 + \V(\beta_{jd})\] + \lambda_d.
	\end{align*}
	...
	\begin{align*}
	  ...
	  \E [\log \pi (\bbeta_d | \bgamma_d^2, \sigma_d^2)] + 
	  \log \pi (\sigma_d^{2}) \\
	  ...
	  \sum_{j=1}^p \E(\gamma_{jd}^{-2})\E(\beta_{jd}^2) \right\} \sigma_d^{-2}, \\
    ...
    & \,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\, + 
    \trace \{ \diag[\E(\gamma_{jd}^{-2})] \V(\bbeta_d)\} + 
     \E(\bbeta_d \tr) \diag[\E(\gamma_{jd}^{-2})] \E(\bbeta_d ) \bigg\}.
	\end{align*}
...
%   iteration.
  ...
  $e_{jd}$ and $b_{jd}$, which in turn depend on the quantities 
  $\diag(\bSigma_d)$, $\bmu_{d}$, $\y_d \tr \X \bmu_d$, 
  $\trace(\X \tr \X \bSigma_d)$, $\bmu_d \tr \X \tr \X \bmu_d$,
  $\trace[\diag(b_{jd})\bSigma_d]$, and $\bmu_d \tr \diag(b_{jd}) \bmu_d$.
  Let $\mathbf{H}_d=\diag[(b_{jd}^{(h)})^{-1}]$. Then, the first two quantities 
  are more efficiently calculated as:
  ...
  $\trace[\diag(b_{jd})\bSigma_d]$, and $\bmu_d \tr \diag(b_{jd}) \bmu_d$ are
  easily calculated from the first two. For the remaining one we have:
  ...
  \section{EB updates}
  For the scale parameter $\lambda_d$ updates to be non-negative, we have to 
  show that $\forall j$:
  \begin{equation}\label{eq:inequality}
    b_{jd}^{(l)} + e_{jd} [(\mathbf{c}_d^j) \tr \bm{\alpha}]^2 - 
    2[(\mathbf{c}_d^j) \tr \bm{\alpha}] = 
    \E_{Q^{(l)}}(\gamma_{jd}^{-2}) + \E_{Q^{(l)}}(\gamma_{jd}^{2}) 
    [(\mathbf{c}_d^j) \tr \bm{\alpha}]^2 - 
    2[(\mathbf{c}_d^j) \tr \bm{\alpha}] \geq 0.
  \end{equation}
  We first note that through Jensen's inequality 
  $\E_{Q^{(l)}}(\gamma_{jd}^{2})^{-1} \leq \E_{Q^{(l)}}(\gamma_{jd}^{-2})$,
  we may lowerbound the left-hand side of (\ref{eq:inequality}):
  $$
  \E_{Q^{(l)}}(\gamma_{jd}^{-2}) + \E_{Q^{(l)}}(\gamma_{jd}^{2}) 
  [(\mathbf{c}_d^j) \tr \bm{\alpha}]^2 - 
  2[(\mathbf{c}_d^j) \tr \bm{\alpha}] \geq 
  \E_{Q^{(l)}}(\gamma_{jd}^{2})^{-1} + \E_{Q^{(l)}}(\gamma_{jd}^{2}) 
  [(\mathbf{c}_d^j) \tr \bm{\alpha}]^2 - 
  2[(\mathbf{c}_d^j) \tr \bm{\alpha}], 
  $$
  ...
  [(\mathbf{c}_d^j) \tr \bm{\alpha}]^2 - 
  2[(\mathbf{c}_d^j) \tr \bm{\alpha}] & \geq 0 \\
  1 + \E_{Q^{(l)}}(\gamma_{jd}^{2})^2 
  [(\mathbf{c}_d^j) \tr \bm{\alpha}]^2 - 
  2 \E_{Q^{(l)}}(\gamma_{jd}^{2}) [(\mathbf{c}_d^j) \tr \bm{\alpha}] & \geq 0 \\
  \left\{ 1 - \E_{Q^{(l)}}(\gamma_{jd}^{2}) [(\mathbf{c}_d^j) \tr \bm{\alpha}]
  \right\}^2 \geq 0.
  \end{align*}
...
@
  ...
  $\eta_d^{(l)}, \lambda_d^{(l)}=\eta_d^{(l + 1)}, \lambda_d^{(l + 1)}$.
	...
	\section{Log pseudo marginal likelihood}\label{sec:lpml}
	The log pseudo marginal likelihood is calculated as:
	$\text{LPML}_d = n^{-1}\sum_{i=1}^n \log (\text{CPO}_{id})$ 
  \cite[]{gelfand_model_1992}. Let 
  $\theta_{id}=\x_i \tr \bbeta_d$ and $\y_{-id}$ denote $\y_{d}$ with 
  observation $y_{id}$ excluded. Then the conditional 
  predictive ordinates (CPO) are given by:
  \begin{align*}
    \text{CPO}_{id} & = p(y_{id} | \y_{-id}) = 
    \int_{-\infty}^{\infty} \int_0^{\infty}
    p(y_{id} | \bbeta_d, \sigma_d^2 ) \pi (\bbeta_d | \y_{-id}, \sigma_d^2) 
  ...
  where
  \begin{align}
    y_{id} | \theta_{id}, \sigma_d^2 & \sim \mathcal{N} (\theta_{id}, 
    \sigma_d^2), \nonumber \\
    \theta_{id} | \y_{-id} & \sim \mathcal{N}(\x_i \tr \bmu_{-id}, 
    \x_i \tr \bSigma_{-id} \x_i), \nonumber \\
    \sigma_d^2 | \y_{-id} & \sim \Gamma^{-1} \( \frac{n + p}{2},
    \zeta_{-id} \), \label{eq:gammavb}
  \end{align}
  where $\bmu_{-id}$, $\bSigma_{-id}$, and $\zeta_{-id}$ are the variational
  parameters estimated from $\y_{-id}$. Plugging in the densities we see that 
  we have:
  \begin{subequations}
    \begin{align}
      \text{CPO}_{id} = & (2 \pi)^{-1} \frac{\Gamma\(\frac{n + p + 1}
      {2}\)}{\Gamma\(\frac{n + p}{2}\)}
      (\zeta_{-id} \x_i \tr \bSigma_{-id} \x_i)^{-1/2} \label{eq:cpo1} \\
      & \cdot \int_{-\infty}^{\infty}
      \( \frac{\theta^2}{2\zeta_{-id}} + 1 \)^{-\frac{n + p + 1}{2}}
      \exp \[ - \frac{(\theta - \x_i \tr \bmu_{-id} + y_i)^2}
      {2 \x_i \tr \bSigma_{-id} \x_i} \] d\theta, \label{eq:cpo2}
    \end{align}
  \end{subequations}
  a one-dimensional (well-behaved) integral that is easily solved with some 
  numerical integration routine.  
  
  In practice we would like to avoid re-estimating the model for every 
  observation $i$. Instead we divide the observations into $K$ parts and 
  estimate $K$ models, so that now $\y_{-id}$ denotes $\y$ with all observations
  in part $k$ removed. Additionally, we add $1 - n_{id}$ to the numerator of the
  Gamma distribution in (\ref{eq:gammavb}), the numerators in the Gamma 
  functions in (\ref{eq:cpo1}), and the numerator of the power in
  (\ref{eq:cpo2}), where $n_{id}$ denotes the number of elements in the removed 
  part.
