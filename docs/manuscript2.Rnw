...
\begin{document}
	...
	\dots & x_{ip} \end{bmatrix} \tr$. Furthermore, we have 
	...
	\begin{subequations}\label{eq:likelihood}
		\begin{align}
	...
	\begin{subequations}\label{eq:prior}
		\begin{align}
		  \beta_{jd} | \gamma^2_{jd}, \sigma^2_d & \sim \mathcal{N}_p 
		  (0, \gamma_{jd}^2 \sigma_d^2), 
		  \label{eq:betaprior}\\
	...
	with mean $\theta$ and shape $\lambda$. The external 
	outcome-feature data comes in through 
	our mean model for the $\gamma_{jd}^2$ hyperprior model: 
	$[(\mathbf{c}_d^j) \tr \bm{\alpha}]^{-1}$. The
	$\bm{\alpha}$ and $\lambda_d$ are estimated in a data-driven manner. 
	More details on the model
	itself, as well as its estimation, may be found in 
	the SM and in [! ref to first paper here].
	
	\subsection{Tissue effect}
	The cell lines used in drug response models are often taken from different
	tissues. In addition, other clinical covariates are often available. 
	For unbiased omics feature effects, one may wish to account for these, 
	without shrinkage of their effects. We do that by adding such unpenalized 
	covariates 
	$\mathbf{z}_{i} = \begin{bmatrix} z_{i1} & \cdots & z_{iT} \end{bmatrix} \tr$ 
	to model (\ref{eq:likelihood}):
  \begin{align*}
    y_{id} & = \x_i \tr \bbeta_d + \mathbf{z}_i \tr \bm{\psi} + 
    \epsilon_{id}, \\
    \text{with } & \epsilon_{id} \sim \mathcal{N}(0, \sigma_d^2),
	\end{align*}
	with $\bm{\psi} = \begin{bmatrix} \psi_1 & \cdots & \psi_T \end{bmatrix} \tr$ 
	the unpenalized covariates effects. We assign $\bm{\psi}$ a flat prior, which 
	alters the estimation as described in SM Section [! ref to section here].
