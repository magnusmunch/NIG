...
@
	...
	fixed. In addition, we estimate regular lasso and ridge models. The null model
	that fits the means of the $\y_d$ is included as a reference.
	
	% The bSEM model is similar to 
