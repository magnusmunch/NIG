...
@
	...
	IC50 estimates for for 251 drugs. Differences in the number of cell lines
...
# res <- as.matrix(res[, -1])
# rownames(res) <- temp
res <- matrix(1, nrow=2510, ncol=13); rownames(res) <- paste0("pmse.", 1:nrow(res))

pmse3 <- res[substr(rownames(res), 1, 5)=="pmse.", ]
pmse3 <- aggregate(pmse3, by=list(rep(1:10, each=251)), FUN="mean")[, -1]
mpmse3 <- apply(pmse3, 2, median)

# res <- read.table("results/analysis_gdsc_res4.txt", row.names=NULL)
# temp <- res[, 1]
# res <- as.matrix(res[, -1])
# rownames(res) <- temp
res <- matrix(1, nrow=2510, ncol=17); rownames(res) <- paste0("pmse.", 1:nrow(res))

pmse4 <- res[substr(rownames(res), 1, 5)=="pmse.", ]
pmse4 <- aggregate(pmse4, by=list(rep(1:10, each=251)), FUN="mean")[, -1]
mpmse4 <- apply(pmse4, 2, median)

tab5 <- rbind(c(mpmse1[1:6], NA, mpmse1[7:length(mpmse1)], rep(NA, 3)),
...
colnames(tab5) <- c("NIG$_{\\text{f}+\\text{d}}^-$", 
                    ...
                    "xtune", "EB ridge", "bSEM")
tab5 <- round(tab5, 3)
tab5 <- t(apply(tab5, 1, function(r) {
...
kableExtra::kable_styling(knitr::kable(
  tab5, align="r",
  caption="Cross-validated PMSE for GDSC data (best performing model in bold). In analyses 1 and 3, bSEM could not be estimated due to non-binary external covariates.",
  format="latex", booktabs=TRUE, escape=FALSE), 
  latex_options=c("HOLD_position"))
@
