...
@
  ...
  \url{https://github.com/magnusmunch/cambridge}.
	...
	100 drug sensitivities for 1014 cell lines are 
	...
	\begin{itemize}
	  ...
	  \item{Scenario 4 is equal to Scenario three, except that the rows of the
	    external covariates matrix are permuted. Permutation
	    adds noise and is supposed to mimic a low external covariates signal. We
	    mimic different noise levels by permutation of fractions 
	...
	NIG$_{\text{f}+\text{d}}^-$. All models are fit on 100 randomly selected cell
	lines. We repeat every simulation Scenario 100 times. 
	
	SM Section \ref{sm-sec:simulations} contains
	additional simulation results for
	the NIG model, plus results for 
	four alternative methods: frequentist (i) lasso and (ii) ridge models, 
	and two EB versions of the ridge model: (iii) \texttt{xtune} 
	\cite[]{ni_bayesian_2019} and (iv) its
	multivariate Bayesian extension as introduced in SM Section 
	\ref{sm-sec:altprior}.
...
@	
  Figure \ref{fig:simulations_gdsc_est2}a shows the estimated
	...
	The median $\chi$ estimate in the 
...
@
  In Figure \ref{fig:simulations_gdsc_est3} the 
	...
	and $\chi$ with median estimates \Sexpr{round(est.phi, 2)} and 
...
@
  ...
  Figure \ref{fig:simulations_gdsc_est4} displays the median $\phi_{jd}$ and 
...
<<simulations_gdsc_est4, fig.cap="Simulation results for Scenario 3: estimated prior means (a) $\\phi_{jd}$, and (b) $\\chi_{d}$ for different levels of noise in the external covariates.", out.width="100%", fig.asp=1/2>>= 
@
  ...
  variance. Underestimation of variance components in regression is 
	...
	by the overestimation of the $\phi_{jd}$, such that the estimated median 
	...
	in practice.
<<simulations_gdsc_var, fig.cap="Simulation results for Scenario 3: estimated prior variances $\\V(\\beta_{jd})$ versus true values, with line of identity (dotted).", out.width="50%", fig.asp=1>>= 
@
	...
	estimates for between 388 and 1043 cell lines on 251 drugs. The number
	of cell lines differs between drugs, because not all drug and cell line 
	...
	from the mutations, obtained from a $t$-test comparing IC50 values of 
	mutated cell lines to unmutated genes. We expect that lower mutation 
	$p$-values result in a larger prior $\beta_{jd}$ variances.
	...
	\begin{itemize}
	  \item{Analysis 1 includes gene expressions as predictors and $p$-values from
	    the gene mutation data as external covariate. A pre-processing step 
	    selects between 221 and 280 genes per drug for which a mutation $p$-value
	    is available.}
	  \item{Analysis 2 includes both gene  expressions and mutations as predictors
	    ...
	    is a expression or mutation. For this analysis, we pre-select 300 gene 
	    expressions with maximum 
	    variance and 295 gene mutations for which there are mutated as well as 
	    unmutated cell lines available.}
	  ...
	  \item{Analysis 4 includes the gene expressions as features and both the
	    ...
	    covariates. We pre-select 500 genes expressions based on maximum 
	    variance.}
	\end{itemize}
	...
	frequentist (iii) lasso and (iv) ridge models, (v) \texttt{xtune}, and
	(vi) its multivariate Bayesian extension (see SM Section 
	\ref{sm-sec:altprior}), named EBridge. In addition, in Analyses 2 and 
	4, we fit a (vii) bSEM model 
	...
	presented in Section \ref{sec:results}. Prediction PMSE,
	$\text{PMSE} = D^{-1} n^{-1} \sum_{d=1}^D 
  \sum_{i=1}^n (\mathbf{y}_d - \mathbf{x}_i \tr \hat{\bm{\beta}_d})^2$, is
  estimated by 10-fold cross validation. In the NIG and bSEM models that provide
  the full posteriors, the posterior mean 
...
res1 <- read.table("results/analysis_gdsc_res1.txt", row.names=NULL)
temp <- res1[, 1]
res1 <- as.matrix(res1[, -1])
rownames(res1) <- temp

pmse1 <- res1[substr(rownames(res1), 1, 5)=="pmse.", ]
pmse1 <- aggregate(pmse1, by=list(rep(1:10, each=251)), FUN="mean")[, -1]
mpmse1 <- apply(pmse1, 2, median)
...
res2 <- read.table("results/analysis_gdsc_res2.txt", row.names=NULL)
temp <- res2[, 1]
res2 <- as.matrix(res2[, -1])
rownames(res2) <- temp

pmse2 <- res2[substr(rownames(res2), 1, 5)=="pmse.", ]
pmse2 <- aggregate(pmse2, by=list(rep(1:10, each=251)), FUN="mean")[, -1]
mpmse2 <- apply(pmse2, 2, median)
...
tab2 <- rbind(c(mpmse1[1:6], NA, mpmse1[7:length(mpmse1)], rep(NA, 3)),
              mpmse2)[, 1:7]
rownames(tab2) <- paste0("model ", 1:nrow(tab2))
colnames(tab2) <- c("NIG$_{\\text{f}+\\text{d}}^-$", 
                    "NIG$_{\\text{f}+\\text{d}}$", "ridge", "lasso", 
                    "xtune", "EB ridge", "bSEM")
tab2 <- round(tab2, 3)
tab2 <- t(apply(tab2, 1, function(r) {
  id <- which(r==min(r, na.rm=TRUE))
  r[id] <- paste0("\\textbf{", r[which(r==min(r, na.rm=TRUE))], "}")
  r}))
@
...
kableExtra::kable_styling(knitr::kable(
  ...
  caption="$\\bm{\\alpha}$ estimates from model 1. Empty cells correspond to fixed zero parameters",
...
kableExtra::kable_styling(knitr::kable(
  tab2, align="r",
  caption="Cross-validated PMSE for GDSC data (best performing model in bold).",
  format="latex", booktabs=TRUE, escape=FALSE), 
  latex_options=c("HOLD_position"))
@
