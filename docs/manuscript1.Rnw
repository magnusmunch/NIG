...
@
  ...
  feature co-data $\mathbf{z}_j$. We will refer to this model as the normal
  inverse Gaussian-inverse Gaussian (NIGIG) model. See SM Section 
  \ref{sm-sec:nigigprior} for more details on this prior.
  ...
  \section{General model 2}
  We have co-data $\mathbf{c}_{jd}$ available that describes the relationship
  between the covariate $j$ and outcome $d$. An example from eQTL is the 
  distance between SNP and gene. Model (\ref{eq:priormodel}) is then rewritten
  as:
  \begin{subequations}\label{eq:generalmodel2}
    \begin{align*}
      y_{id} | \bm{\beta}_d, \sigma_d^2 & \sim \mathcal{N} 
      \(\x_i \tr \bm{\beta}_d, \sigma_d^2\), \nonumber \\
      ...
      \( (\mathbf{c}_{jd} \tr \bm{\alpha})^{-1}, \lambda_{jd} \), \\
      \sigma_d^2 & \sim 1/\sigma_d^3, \nonumber
    \end{align*}
  \end{subequations}
  ...
  The formulation as in (\ref{eq:generalmodel2}) results in the following
  variational distribution for the $\gamma^2_{jd}$:
  $$
  q(\gamma_{jd}^2) \overset{D}{=} \mathcal{GIG}
  \(-1, \lambda_{jd}(\mathbf{c}_{jd} \tr \bm{\alpha})^2, \delta_{jd}\).
  $$
  The variational parameters are now updated by:
  \begin{align*}
    \bm{\Sigma}_d^{(h+1)} & = (a_d^{(h)})^{-1} 
    ...
    (\bSigma_d)_{jj}\] + \lambda_{jd}, \\ 
    \zeta_d^{(h+1)} & = \frac{1}{2} \bigg[\mathbf{y}_d \tr \mathbf{y}_d -
    2 \mathbf{y}_d \tr \X \bm{\mu}_d^{(h+1)} + 
    \trace ( \X \tr \X \bm{\Sigma}_d^{(h+1)}) + 
    (\bm{\mu}_d^{(h+1)}) \tr \X \tr \X \bm{\mu}_d^{(h+1)} \\
    & \,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\, + 
    \trace \[ \diag (b_{jd}^{(h)}) \bm{\Sigma}_d^{(h+1)}\] + 
    (\bm{\mu}_d^{(h+1)}) \tr \diag (b_{jd}^{(h)}) 
    \bm{\mu}_d^{(h+1)}\bigg],
  \end{align*}
  until convergence, where $a_d^{(h)}$ is as before and 
  $$
  b_d^{(h)}=\sqrt{\frac{\delta_{jd}}
  {\lambda_{jd}(\mathbf{c}_{jd} \bm{\alpha})^2}}
  \frac{K_0(\sqrt{\delta_{jd}\lambda_{jd}(\mathbf{c}_{jd} \bm{\alpha})^2})}
  {K_1(\sqrt{\delta_{jd}\lambda_{jd}(\mathbf{c}_{jd} \bm{\alpha})^2})} + 
  \frac{2}{\delta_{jd}}.
  $$ 
  The empirical Bayes step that consists of an iteratively 
  reweighted least squares regression with steps:
  \begin{align*}
    \bm{\alpha} & = \[ \mathbf{C} \tr \diag 
    (\lambda_{jd} e_{jd}^{(l)}) \mathbf{C} \]^{-1} \mathbf{C} \tr \diag 
    (\lambda_{jd}) \mathbf{1}_{p \cdot D \times 1}, \\
    \lambda_{jd} & = \[ b_{jd}^{(l)} + e_{jd}^{(l)} 
    (\mathbf{c}_{jd} \tr \bm{\alpha})^2 - 
    2\mathbf{c}_{jd} \tr \bm{\alpha} \]^{-1},
  \end{align*}
  where $e_{jd}^{(l)} = \E_{Q^{(l)}}(\gamma_d^2) = [b_{jd}^{(l)} - 
  2/\delta_{jd}^{(l)})] \cdot \delta_{jd}^{(l)} 
  /[\lambda_d^{(l)} (\mathbf{c}_{jd} \tr \bm{\alpha})^2]$.
  
  Note that so far, the design matrix $\X$ is assumed to be shared across the
  equations. This is not a requirement, so that we may have $\X^d$, for 
  $d=1, \dots, D$ different design matrices. As a consequence, the index $j$
  may run over differently sized sequences.

  \section{Simulations}
  \subsection{Comparison of NIG, NIGIG and Student's \texorpdfstring{$t$}{t} 
  priors}
  ...
  We compare the 
  posteriors of the proposed NIG and NIGIG priors to the scaled Student's 
...
@
  ...
  \begin{align*}
    ...
    \beta_j | z_j & \sim z_j \mathcal{N} \( 0, 1\) +
    (1 - z_j)\delta_0,
  \end{align*}
  where $\delta_0$ denotes the Dirac mass at zero. The slab is chosen
  ...
  $\beta_j$ is:
  $$
  \beta_j \sim (s - 1)\mathcal{N} \( 0, 1\) + s\delta_0.
  $$
...
@
  ...
  - metabolomics data: oxidative stress als uitkomst van de andere metabolieten
