...
@
  ...
  \url{https://github.com/magnusmunch/cambridge}.
	...
	estimated (feature, drug, or both),
...
tab <- cbind(apply(res[rownames(res)=="emse", ], 2, median),
...
colnames(tab) <- c("$\\text{EMSE}_{\\text{test}}$",
                   "$\\text{PMSE}_{\\text{test}}$")     
@	
...
@
	...
	Table \ref{tab:simulations_gdsc_tab1} displays median estimation MSE (EMSE) 
	and prediction MSE (PMSE) for simulation version 1. The table includes as a
	reference the null model, which
...
kable(tab, col.names=colnames(tab), align="r",
      caption="Median performance measure estimates for simulation version 1 (lowest in bold).", 
...
@	
  Figure \ref{fig:simulations_gdsc_est2} shows the estimated
	...
	in version 2 of the simulation study. They are estimated without bias.
...
@
	...
	the same as in Section \ref{sec:simulations}, so NIG$_{\text{f}+\text{d}}^-$
	does not use the external covariates and estimates
...
@
  ...
  drugs. On the other hand, the NIG$_{\text{d}}$ model, that only includes
  external
...
@
  ...
  \Sexpr{round(tab[5, 1], 3)}. The underperformance of the 
...
@	
	...
	beneficial for drug sensitivity prediction. PMSE of the NIG
	model with both both feature and drug variance components is lower
	than for the regular lasso model and although the PMSE is
	...
	PMSEs are almost uniformly lower over the drugs for the NIG model 
	compared to bSEM. Consequently, the total PMSE is significantly
	lower for the NIG model. 
	
	A possible direction of future research that attempts 
	...
	additional constraint that the drug and feature variance components have to 
	average to one, this cross-validated variance component may be interpreted as
	the overall level of shrinkage that is then tailored to lower PMSE.
