...
@
  ...
  \url{https://github.com/magnusmunch/cambridge}.
	...
	the all external covariates, called 
	NIG$_{\text{f}}$, NIG$_{\text{d}}$, or
	NIG$_{\text{f}+\text{d}}$, again depending on which variance components are 
	estimated, and regular (iii) lasso and (iv) ridge models.
	...
	measures are estimated from the remaining 241 cell lines. We repeat the whole
	simulation 50 times.
