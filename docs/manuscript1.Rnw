...
@
	...
	is an empty model that only fits the mean of the $\y_{id}$.
	...
	external covariates, according to the ELBO on the training data.
...
@
	...
	drug separately, the 50 features that are not in the pathway of the drug with
	the largest variance and the 50 features that are in the pathway of the drug
	with the largest variance. If less than 50 features that are in the pathway 
	are available, we select the maximum number available. This strategy ensures
	...
	three different analysis outcomes.
	...
	variables, named NIG1, NIG2, NIG3, and NIG4 in the following. NIG1 estimates
	only one overall $\phi$ and one overall $\chi$, NIG2 uses the external 
	...
	fixed. In addition, we estimate regular lasso and ridge models.
