...
\begin{document}
  ...
  finance (see, e.g., \cite{kalemanova_normal_2007}). In Figure 
...
@
  ...
  \section{Simulations}
  \subsection{Comparison of NIG to normal inverse Gamma model}
  ...
  We compare the 
  posteriors of the proposed NIG prior to the scaled Student's $t$-distribution
  prior in a simulated setting. We generate draws from these posteriors using 
  an MCMC sampler implemented in \texttt{rstan}. The aim of this simulation 
  study is 
  to investigate the performance of the NIG prior model, so we set $D=1$ and 
  drop the $d$ indices in the following.
  ...
  several measures. Divergence between the MCMC and variational posterior
  ...
  indicate more similar distributions. In addition, we calculate the correlation
  and mean squared distance (MSD) between the posterior variances of
  the MCMC posterior and variational posterior. \texttt{rstan} allows the user 
  ...
  so we restrict ourselves to the univariate setting (i.e., $D=1$) and drop any
  $d$ indices from notation in the following. We simulated
  $n=100$ features $\x_i$ from a $p=200$ dimensional Gaussian distribution with
  zero mean vector, variances equal to one, and all covariances set to 
  $\rho=0.5$. The outcomes are generated through $y_i = \x_i\tr \bbeta + 
  \epsilon_i$, with the $\epsilon_i$ from the standard normal distribution. 
  Likewise, we generate the model parameters $\beta_j$ from a standard normal
  distribution. For estimation, we set our hyperparameters $\lambda=0.1$ and 
  ...
  $1/(n\sigma^2\theta)=1/200$.To mitigate the effect of random variation, we
  repeat the simulation 100 times and report the results in Figures 
