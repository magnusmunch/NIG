...
@
  ...
  estimates of the model parameters. We compare these MAP estimates together
  with the ridge estimates obtained from the \texttt{glmnet} \texttt{R} package
  \cite[]{friedman_regularization_2010}, and the ADVI posterior means,
  ...
  repeated the simulations 100 times and report the results in Figure 
  \ref{fig:boxplots_igaussian_res4.2_post1}.
  
<<boxplots_igaussian_res4.2_post1, fig.cap="(a) \\& (d) Cramér-von Mises criterion, (b) \\& (e) correlation, and (c) \\& (f) MSD between the $\\beta_j$ point estimates of the MCMC and variational bayes posteriors.", out.width="80%", fig.asp=2/3>>=
@
