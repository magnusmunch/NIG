...
@
  ...
  \subsection{Alternative prior}\label{sec:altprior}
  Consider the following simpler hyperprior alternative to (\ref{eq:prior}):
  \begin{subequations}\label{eq:altprior}
	...
	variance follows the Jeffrey`s prior from (\ref{eq:prior}). Throughout the
	following, whenever $\zeta$ and $\nu$ are not specified, we use $\zeta \to 0$ 
	and $\nu=1/2$.
	
	Estimation of $\bm{\alpha}_{\text{feat}}$ and $\bm{\alpha}_{\text{feat}}$, and
	ultimately $\bm{\beta}_d$ is explained in Section \ref{sm-sec:altprior} of the
	SM. The results in Sections \ref{sec:simulations} and \ref{sec:gdsc}
	include model (\ref{eq:altprior}) as an alternative to (\ref{eq:prior}).
...
ranges <- matrix(1, nrow=10, ncol=10)
tab <- matrix(1, nrow=10, ncol=10)
tabm <- matrix(1, nrow=10, ncol=10)
kurts <- matrix(1, nrow=10, ncol=10)
@
...
@
  ...
  The choice to model the $\bm{\alpha}$`s, as hyperparameters instead of
  full Bayesian parameters is three-fold: (i) it allows fast and stable 
  empirical Bayes estimation as compared to badly mixing (see Comment 5)
  full Bayesian MCMC sampling. (ii) \cite{wiel_learning_2019} shows that in a 
  somewhat similar model, empirical Bayes is competitive to full Bayesian 
  inference in terms of frequentist coverage of credible intervals, i.e.,
  uncertainty propagation of the hyperparameters is not always necessary.
  Additionally, (iii) in SM Section \ref{sm-sec:gibbssampler} a full Bayes
  Gibbs sampler is described, where the full conditional of the $\bm{\alpha}$ is
  Gaussian. For vague $\bm{\alpha}$ and $\lambda$ priors, the mean of this 
  Gaussian coincides 
  with the empirical Bayes updates as described in the MD, a strong 
  indication that the posterior point estimates of our EB approach and the full
  Bayes model are similar.
