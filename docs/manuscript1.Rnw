...
\begin{document}
  ...
  this prior), ridge and lasso, all scaled to variance one. From this Figure 
  we see that, depending on the choice
...
@
  ...
  objective (up to model specification). 
	...
	in an objective, data-driven manner. Additionally, it reduces the number of
	hyperparameters even further. The estimating equations are:
  \begin{align*}
  ...
  \begin{subequations}\label{eq:generalmodel}
    \begin{align}
      y_{id} | \bm{\beta}_d, \sigma_d^2 & \sim \mathcal{N} 
      (\x_i \tr \bm{\beta}_d, \sigma_d^2), \nonumber \\
      \beta_{jd} | \tau_j^2, \gamma_d^2, \sigma_d^2 & \sim \mathcal{N} 
      (0, \sigma_d^2 \gamma_d^2 \tau_j^2), \\
      \gamma_d^2 & \sim \mathcal{IG}\(\theta_d^c, \lambda^c_d\), \\
      \tau_j^2 & \sim \mathcal{IG}\(\theta_j^z, \lambda^z_j\), \\
      \sigma_d^2 & \sim 1/\sigma_d^3, \nonumber
    \end{align}
  \end{subequations}
  ...
  feature co-data $\mathbf{z}_j$. Equivalently, we may set 
  $\eta_{jd}^2=\tau_j^2 \gamma_d^2/(\theta_j^z \theta_d^c)$ and write 
  (\ref{eq:generalmodel}) as:
  \begin{align*}
  ...
  \begin{subequations}\label{eq:generalprior}
    ...
    \end{align}
  \end{subequations}
  From (\ref{eq:generalprior}) we find the marginal variance and kurtosis for 
  $\beta_{jd}$ (conditional on $\sigma_d^2$):
  \begin{align}
    \V(\beta_{jd} | \sigma_d^2) & =\sigma_d^2 \theta_j^z \theta_d^c 
    \[\frac{K_0(\lambda_d^c/\theta_d^c)}{K_1(\lambda_d^c/\theta_d^c)} + 
    \frac{2\theta_d^c}{\lambda_d^c}\],\nonumber \\
    \mathcal{K}(\beta_{jd} | \sigma_d^2) & = 3 \(\frac{\theta_j^z}{\lambda_j^z}
    + 1\) \theta_d^c \[r_d - r_d^{-1} + 8\(\frac{\theta_d^c}{\lambda_d^c}\)^2
    K_0 (\lambda_d^c/\theta_d^c) + \frac{4 \theta_d^c}{\lambda_d^c}r_d 
    K_0 (\lambda_d^c/\theta_d^c) + 1/\theta_d^c\], \label{eq:kurtnigig}
  \end{align}
  with $r_d = K_0(\lambda_d^c/\theta_d^c)/K_1(\lambda_d^c/\theta_d^c)$. If 
  ...
  \rightarrow 3(\theta_j^z/\lambda_j^z + 1)$. From (\ref{eq:kurtnigig}) we see
  ...
  $\lambda_j^z$. This makes the model (\ref{eq:generalmodel}), which we 
  refer to by the NIGIG model hereafter, a flexible model.
  
  The formulation as in (\ref{eq:generalmodel}) results in an additional 
  variational distribution for the $\tau_j$:
  $$
  q(\tau_j^2) \overset{D}{=} \mathcal{GIG}\(-\frac{D+1}{2}, \lambda^z_j 
  (\theta_j^z)^{-2}, \delta_j^z\).
  $$
  ...
  \begin{align*}
    ...
    \X \tr \y_d, \\
    (\delta^c_d)^{(h+1)} & = a_d^{(h)} \left\{(\bmu^{(h+1)}_d) \tr 
    \diag (c_j^{(h)}) \bmu^{(h+1)}_d + \trace \[ \diag (c_j^{(h)}) 
    \bSigma_d^{(h+1)}\] \right\} + \lambda^c_d, \\ 
    (\delta^z_j)^{(h+1)} & = \sum_{d=1}^D a_d^{(h)} b_d^{(h)} 
    ...
    (\bm{\mu}_d^{(h+1)}) \tr \X \tr \X \bm{\mu}_d^{(h+1)} \\
    & \,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\, + 
    b_d^{(h+1)} \trace \[ \diag (c_j^{(h)}) \bm{\Sigma}_d^{(h+1)}\] + 
    b_d^{(h+1)} (\bm{\mu}_d^{(h+1)}) \tr \diag (c_j^{(h)}) 
    \bm{\mu}_d^{(h+1)}\bigg],
  \end{align*}
  until convergence, where
  $$
  c_j^{(h)} = \E_{Q^{(h)}}(\tau_j^{-2}) = \sqrt{\frac{\lambda^z_j}
  {(\theta_j^z)^2 (\delta^z_j)^{(h)}}} 
  \frac{K_{\frac{D - 1}{2}} 
  \( \sqrt{\lambda^z_j (\delta^z_j)^{(h)}}/\theta_j^z \)}
  {K_{\frac{D+1}{2}} \( \sqrt{\lambda^z_j (\delta_j^z)^{(h)}}/\theta_j^z \)} +
  \frac{D+1}{(\delta_j^z)^{(h)}}
  $$
  If we set $\theta_d^c=(\mathbf{c}_d \tr \bm{\alpha}^c)^{-1}$ and
  $\theta_j^z=(\mathbf{z}_j \tr \bm{\alpha}^z)^{-1}$, we have an additional
  empirical Bayes step that consists of an iteratively 
  reweighted least squares regression of responses $(f_j^{(l)})^{-1}$ on
  predictors $\mathbf{z}_j$ with weights $\lambda_j^z f_j^{(l)}$, where 
  \begin{align*}
  f_j^{(l)} & = \E_{Q^{(h)}}(\tau_j) = [c_j^{(l)} - 
  (D + 1)/(\delta_j^z)^{(l)}] \cdot (\delta_j^z)^{(l)} [\mathbf{z}_j \tr 
  (\bm{\alpha}^z)^{(l)} ]^{-2} / (\lambda_j^z)^{(l)}, \\
  \lambda_j^z & = \[ c_j^{(l)} + f_j^{(l)} 
  (\mathbf{z}_j \tr \bm{\alpha}^z)^2 - 
    2\mathbf{z}_j \tr \bm{\alpha}^z \]^{-1}.
  \end{align*}

  \section{Simulations}
  \subsection{Comparison of NIG and Student's \texorpdfstring{$t$}{t} priors}
  ...
  $$
  with shape $\eta_d/2$ and scale $\lambda_d/2$, both rescaled to match our 
  ...
  t\(\eta_d,\sigma_d \sqrt{\lambda_d/\eta_d}\),
  $$
  with degrees of freedom $\eta_d$ and scale $\sigma_d \sqrt{\lambda_d/\eta_d}$.
  See SM Section \ref{sm-sec:inversegamma} for more details on this prior. 
  We compare the 
  posteriors of the proposed NIG prior and the scaled Student's $t$-distribution
  prior in a simulated setting. 
  
  We compare the two posterior distributions to the true posterior for a 
  ...
  distribution. For estimation, we set our hyperparameters $\lambda=0.5$, 
  $\theta=1$, and $\eta=2.5$. such that the marginal $\beta_j$ variances
  of the NIG and Student's $t$ priors match. For both priors, we generated 
  1000 draws (after 1000 burnin) from the posteriors using an MCMC sampler 
  implemented in \texttt{rstan}. 
  
  In the sparse setting, the observed data generating process is the same. The
  model parameters are are simulated from a spike-and-slab prior with a 
  Laplace slab:
  \begin{align*}
    w & \sim \text{Beta}(\nu, \kappa), \\
    z_j | w & \sim \text{Binom}(w), \\
    \beta_j | z_j & \sim z_j\text{Laplace}
    \(\sqrt{\frac{\nu + \kappa}{2\nu}} \sigma \gamma\) + (1 - z_j)\delta_0,
  \end{align*}
  where $\delta_0$ denotes the Dirac mass at zero. The Laplace scale is chosen
  ...
  $s=1 - \nu/(\nu + \kappa)$. Note that the resulting marginal prior for
  $\beta_j$ is:
  $$
  \beta_j \sim (s - 1)\text{Laplace}\(\sqrt{\frac{\nu + \kappa}{2\nu}} 
  \sigma \gamma\) + s\delta_0.
  $$
  ...
  \ref{fig:boxplots_igaussian_res4_post} and [! ref to figure here]
<<boxplots_igaussian_res4_post, fig.cap="Comparison of the NIG and Student's t priors through (a) Cramér-von Mises criterion between the model and true posteriors, (b) correlation and (c) MSD between the posterior means and true parameters, and (d) 95\\% credible interval coverage of the true values.", out.width="100%", fig.asp=2/5>>=
@
...
@
  ...
  \section{Empirical Bayes estimation}
...
@
  ...
  \section{Application}
  - Possible: network reconstruction, medical surveys, drug response prediction
