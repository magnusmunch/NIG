...
\begin{document}
  ...
  and features in a data-driven manner.
	...
	In model (\ref{eq:prior}), $\gamma_{jd}^2$ denotes a local variance 
	component that is supposed to capture local, feature-specific variation in the
	model parameters $\beta_{jd}$, while the global variance component $\tau$ 
	captures the, drug-specific, general trend in $\bm{\beta}_{d}$. Each drug is
	endowed with a random error variance $\sigma_d^2$.
