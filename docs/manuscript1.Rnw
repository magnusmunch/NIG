...
@
  ...
  \url{https://github.com/magnusmunch/cambridge}.
	...
	and and EB version of the ridge model: (iii) a
...
@
  ...
  are estimated by 10-fold cross validation. In the NIG models that 
...
res <- read.table("results/analysis_gdsc_res4.1.txt", row.names=NULL)
temp <- res[, 1]
res <- as.matrix(res[, -1])
rownames(res) <- temp
