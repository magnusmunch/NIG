...
@
  ...
  \url{https://github.com/magnusmunch/cambridge}.
	...
	All models are fit on 100 randomly selected cell lines. We repeat every
	simulation Scenario 100 times.
	
	SM Section \ref{sm-sec:simulations} contains additional simulation results for
	...
	multivariate Bayesian extension as introduced in MD Section 
	\ref{sm-sec:altprior}.
...
@	
  Figure \ref{fig:simulations_gdsc_est1} shows the estimated
	...
	consistently underestimated. This results in overestimation on the $\phi_{jd}$
	scale as depicted in Figure \ref{fig:simulations_gdsc_est1}b. 
...
@
  ...
  Underestimation of variance components is 
	a common phenomenon in variational Bayes approximations, so the 
	underestimation of the mean of the drug variance components $\chi_d$ in
	Scenarios 2 and 3 is somewhat expected. In Scenario 3, this is compensated for
	by the overestimation of the $\phi_{jd}$. We note that increasing the number
	of cell lines $n$ and increasing the number of drugs $D$ improves estimation
	of the $\chi_d$. However, this is not the most interesting situation since it
