...
@
  ...
  Finally, the results from Scenario 5 in SM Section \ref{sm-sec:simulations} 
  ...
  $\hat{\bm{\beta}_d}$ the estimator for $\bm{\beta}_d$. In the NIG and bSEM 
...
tab4.1 <- t(fit4)[c(1, 2, 7), c(1:2)]
rownames(tab4.1) <- c("NIG$_{\\text{f}+\\text{d}}^-$", 
                    "NIG$_{\\text{f}+\\text{d}}$", "bSEM")
...
tabm5 <- rbind(c(mpmse1[1:6], NA, mpmse1[7:length(mpmse1)], rep(NA, 3)),
               ...
               mpmse4)
tab5.1 <- tab5[, c(1:4, 6:7)]
tabm5.1 <- tabm5[, c(1:4, 6:7)]
...
colnames(tab5.1) <- c("NIG$_{\\text{f}+\\text{d}}^-$", 
                    "NIG$_{\\text{f}+\\text{d}}$", "ridge", "lasso", "mxtune", 
                    "bSEM")
tab5.1 <- t(tab5.1)
...
colnames(tab5.2) <- c("lasso", "NIG$_{\\text{f}+\\text{d}}+$DSS")
tab5.2 <- t(tab5.2)

mpsel <- rbind(c(colMeans(psel1[, 1:6]), NA, colMeans(psel1[, 7:ncol(psel1)]), 
                 rep(NA, 3)), colMeans(psel2), 
               c(colMeans(psel3[, 1:6]), NA, colMeans(psel3[, 7:ncol(psel3)]), 
                 rep(NA, 3)), colMeans(psel4))

# prior variances
vars1 <- round(1/as.numeric(model.matrix(~ factor(c(1:3))) %*% 
                              fit4[c(3, 6, 7), 2]), 4)
vars2 <- round((cbind(1, c(0, 1)) %*% fit2[c(1, 2), c(2, 7)])[1, ]/
                 (cbind(1, c(0, 1)) %*% fit2[c(1, 2), c(2, 7)])[2, ], 3)

# pmse ranges
ranges1 <- apply(rbind(mpmse1[c(1:4, 6:7)], mpmse2[c(1:4, 6:7)], 
                       mpmse3[c(1:4, 6:7)], mpmse4[c(1:4, 6:7)]), 1, range) 
ranges1 <- round(ranges1[2, ] - ranges1[1, ], 4)
psels1 <- round(mpsel[, c(4, 9)], 0)
@
  ...
  \begin{itemize}
	  \item{Analysis 1 results in a positive additive effect of the mutation 
	    $p$-values on the prior $\bm{\beta}_d$ precisions
	    (Table \ref{tab:analysis_gdsc_fit1}). This translates
	    to more $\bm{\beta}_d$ shrinkage with increasing $p$-value,
	    as expected.}
	  \item{Analysis 2 shows that gene mutations are more predictive 
	    ...
	    dummy on prior precions (Table \ref{tab:analysis_gdsc_fit2}): 
	    mutations are shrunken less than
	    expressions.}
	  \item{Analysis 3 indicates that CCLE $p$-values are positively related
	    to prior precision (Table \ref{tab:analysis_gdsc_fit2}), i.e., 
	    higher CCLE $p$-values results in more shrinkage of $\bm{\beta}_d$, as 
	    expected.}
  ...
  In comparison to bSEM, Analysis 2 (Table \ref{tab:analysis_gdsc_fit2}) stands
  out. The ratio of expression prior precision to mutation prior precision
  (ignoring other variance components) is
  $\hat{\E}(\tau_{\text{exp}}^{-2})/\hat{\E}(\tau_{\text{mut}}^{-2}) 
  \approx \Sexpr{vars2[1]}$ for NIG$_{\text{f}+\text{d}}$ and 
  $\hat{\E}(\tau_{\text{exp}}^{-2})/\hat{\E}(\tau_{\text{mut}}^{-2}) 
  \approx \Sexpr{vars2[2]}$ for 
  bSEM. This means that the effects are reversed: bSEM penalized mutations 
  more than expressions, while NIG$_{\text{f}+\text{d}}$ penalized
  expressions more. Furtheremore, the magnitude of the effect is much larger for
  bSEM, indicating that bSEM picks up more signal in the external covariate 
  here.
...
@
  ...
  4, while bSEM outperforms in Analysis 2. In general however, all models
  perform very similar. The ranges of mean PMSE over all methods are
  ...
  Furthermore, the differences between models that include external covariates
  and their respective counterparts that do not, are small, indicating that the
...
@
  ...
  its $\bm{\beta}_d$ prior is heavy-tailed as compared to 
  the ridge prior 
  thus facilitates features selection. Selection of features from a sparse prior
  ...
  analyses, where we select (approximately) the same number of features as 
  lasso. Table 
  \ref{tab:analysis_gdsc_pmse2} compares the resulting mean PMSE to lasso. The
  Table shows that NIG$_{\text{f}+\text{d}}$ plus DSS outperforms lasso,
  with the same number of features (\Sexpr{psels1[-4, 2]}, and 
  \Sexpr{psels1[4, 2]} features in the four analyses, respectively).
...
<<time>>=
times <- round(read.table("results/analysis_gdsc_time1.txt")[, 1], 0)
@
	...
	covariates (NIG$_{\text{f}+\text{d}}$ in Section \ref{sec:gdsc}) in the
	GDSC data analyses in Section \ref{sec:gdsc} took 
	\Sexpr{times[-1]}, and \Sexpr{times[1]} seconds on a 2016 
	...
	the simulation results in Section \ref{sm-sec:simulations} of the SM show that
	inclusion of informative external covariates is 
	still beneficial to predictive performance. 

	The model is put into practice on the GDSC data in Section \ref{sec:gdsc}. 
	The NIG model is competitive with convential, penalized models like lasso in 
  ...
  PMSE of ridge regression is therefore not surprising. 
  However, direct prediction error optimisation by cross-validation is not 
	feasible in the external covariates setting, due to the large number of 
	hyperparameters.
  A caveat with penalized
  ...
  to the parameter uncertainties for a better interpretable model.
	...
	\ref{sec:gdsc} show that the inclusion of the external covariates
