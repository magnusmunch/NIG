...
@
  ...
  \url{https://github.com/magnusmunch/cambridge}.
	...
	\section{Simulations}\ref{sec:simulations}
	\subsection{Setup}
...
kable(tab, col.names=colnames(tab), align="r",
      caption="Median performance measure estimates for simulation version 1, with the best performing model in bold.", 
...
options(knitr.kable.NA="", digits=3)
kable(tab, col.names=colnames(tab), align="r",
      caption="Median performance measure estimates for simulation version 2, with the best performing model in bold.", 
      format="latex", booktabs=TRUE, escape=FALSE) %>%
  kable_styling(latex_options=c("HOLD_position"))
@
...
options(knitr.kable.NA="", digits=3)
kable(tab, col.names=colnames(tab), align="r",
      caption="Median performance measure estimates for simulation version 3, with the best performing model in bold.", 
      format="latex", booktabs=TRUE, escape=FALSE) %>%
  kable_styling(latex_options=c("HOLD_position"))
@
	...
	\subsection{Results}
	As a preliminary step, we select
	features according to three strategies. The first strategy selects for each
	drug separately 50 features that are not in the pathway of the drug with
	maximum variance and 50 features that are in the pathway of the drug
	with maximum variance. If less than 50 features in the pathway are available,
	we select all available features. This strategy ensures
	that the features are somewhat balanced with respect to the external feature
	covariate, i.e., pathway membership. The second strategy selects the 
	100 features with the largest variance, irrespective of pathway membership,
	while the last stategy selects 1000 features with the largest variance,
	again irrespective of pathway membership. These three strategies lead to
	three different analyses described below.
	...
	variables, named NIG1, NIG2, NIG3, and NIG4 in the following. NIG1 does 
	not use the external covariates and estimates
	one overall $\phi$ and one overall $\chi$, NIG2 uses the external 
...
colnames(tab) <- c("$\\text{ELBO}_{\\text{test}}$",
                   ...
                   "$\\text{PMSE}_{\\text{test}}$")  
@
  From the $\bm{\alpha}$ estimates in Tables \ref{tab:analysis_gdsc_est1_feat} 
  and \ref{tab:analysis_gdsc_est1_feat} we see that there
  seems to be an effect of the
  external variables. Models NIG2 and NIG3 estimate a negative effect for the
  pathway external covariate. This translates to a negative additive effect on
  the expected prior precision of the $\bm{\beta}_d$, i.e., genes that are found
  ...
  If we look at the NIG1 model, that includes all external 
...
kable(est1,
      ...
      caption="$\\bm{\\alpha}_{\\text{feat}}$ estimates from GDSC data for feature selection strategy 1.",
      format="latex", booktabs=TRUE, escape=FALSE) %>%
  kable_styling(latex_options=c("HOLD_position"))
@
...
kable(est2,
      ...
      caption="$\\bm{\\alpha}_{\\text{drug}}$ estimates from GDSC data for feature selection strategy 1.",
      format="latex", booktabs=TRUE, escape=FALSE) %>%
  kable_styling(latex_options=c("HOLD_position"))
@
...
<<analysis_gdsc_tab1>>=
kable(tab,
      col.names=colnames(tab), align="r",
      caption="Performance measures from GDSC data for feature selection strategy 1.",
      format="latex", booktabs=TRUE, escape=FALSE) %>%
  kable_styling(latex_options=c("HOLD_position"))
@	
  ...
  . In both NIG models the correlations
...
est1 <- cbind(fit[, 1], c(NA, 0, 0, NA), fit[, 2])
colnames(est1) <- c("intercept", "not in pathway", "in pathway")
est2 <- cbind(fit[, 3:5], c(NA, 0, NA, 0), fit[, c(7:6)], c(NA, 0, NA, 0))
colnames(est2) <- c("intercept", "experimental", "development", "approved",
                    "unkown", "targeted", "cytotoxic")
...
<<analysis_gdsc_est2_feat>>=
options(knitr.kable.NA="", digits=3)
kable(est1,
      col.names=colnames(est1), align="r",
      caption="$\\bm{\\alpha}_{\\text{feat}}$ estimates from GDSC data for feature selection strategy 2.",
      format="latex", booktabs=TRUE, escape=FALSE) %>%
  kable_styling(latex_options=c("HOLD_position"))
@

<<analysis_gdsc_est2_drug>>=
kable(est2,
      col.names=colnames(est2), align="r",
      caption="$\\bm{\\alpha}_{\\text{drug}}$ estimates from GDSC data for feature selection strategy 2.",
      format="latex", booktabs=TRUE, escape=FALSE) %>%
  kable_styling(latex_options=c("HOLD_position"))
@
...
kable(tab,
      col.names=colnames(tab), align="r",
      caption="Performance measures from GDSC data for feature selection strategy 2.",
      format="latex", booktabs=TRUE, escape=FALSE) %>%
  kable_styling(latex_options=c("HOLD_position"))
@	
...
<<analysis_gdsc_est3_feat>>=
options(knitr.kable.NA="", digits=3)
kable(est1,
      col.names=colnames(est1), align="r",
      caption="$\\bm{\\alpha}_{\\text{feat}}$ estimates from GDSC data for feature selection strategy 3.",
      format="latex", booktabs=TRUE, escape=FALSE) %>%
  kable_styling(latex_options=c("HOLD_position"))
@

<<analysis_gdsc_est3_drug>>=
kable(est2,
      col.names=colnames(est2), align="r",
      caption="$\\bm{\\alpha}_{\\text{drug}}$ estimates from GDSC data for feature selection strategy 3.",
      format="latex", booktabs=TRUE, escape=FALSE) %>%
  kable_styling(latex_options=c("HOLD_position"))
@
...
kable(tab,
      col.names=colnames(tab), align="r",
      caption="Performance measures from GDSC data for feature selection strategy 3.",
      format="latex", booktabs=TRUE, escape=FALSE) %>%
  kable_styling(latex_options=c("HOLD_position"))
@	
	...
	further attention. One proposed solution is to include a
