context("vb update without unpenalized covariates")
test_that("low dim. vb update", {
  
  # setting parameters
  n <- 100
  p <- 10
  x <- matrix(rnorm(p*n), nrow=n, ncol=p)
  y <- rnorm(n)
  aold <- rchisq(1, 1) + 1
  bold <- rchisq(p, 1) + 1
  Calpha <- rchisq(p, 1) + 1
  lambda <- rchisq(1, 1) + 1
  ytx <- as.numeric(t(y) %*% x)
  yty <- as.numeric(t(y) %*% y)
  
  # implemented function
  test.vb <- .single.vb.update(aold, bold, Calpha, lambda, y, x, ytx, yty, n, p)
  
  # actual calculation (see supplement)
  Sigma <- solve(t(x) %*% x + diag(bold))/aold
  mu <- as.numeric(solve(t(x) %*% x + diag(bold)) %*% t(x) %*% y)
  delta <- aold*(mu^2 + diag(Sigma)) + lambda
  b <- sqrt(lambda*Calpha^2/delta)*
    ratio_besselK(sqrt(lambda*delta)*Calpha, 1) + 2/delta
  zeta <- 0.5*as.numeric(
    t(y) %*% y  - 2*t(y) %*% x %*% mu + sum(diag(t(x) %*% x %*% Sigma)) +
      t(mu) %*% t(x) %*% x %*% mu + sum(diag(diag(b) %*% Sigma)) + 
      t(mu) %*% diag(b) %*% mu)
  a <- (n + p + 1)/(2*zeta)
  e <- (b - 2/delta)*delta/(lambda*Calpha^2)
  vb <- list(delta=delta, zeta=zeta, a=a, b=b, e=e)
  
  # check
  expect_equal(vb, test.vb)
  
})

test_that("high dim. vb update", {
  
  # setting parameters
  n <- 10
  p <- 100
  x <- matrix(rnorm(p*n), nrow=n, ncol=p)
  y <- rnorm(n)
  aold <- rchisq(1, 1) + 1
  bold <- rchisq(p, 1) + 1
  Calpha <- rchisq(p, 1) + 1
  lambda <- rchisq(1, 1) + 1
  ytx <- as.numeric(t(y) %*% x)
  yty <- as.numeric(t(y) %*% y)
  
  # implemented function
  test.vb <- .single.vb.update(aold, bold, Calpha, lambda, y, x, ytx, yty, n, p)
  
  # actual calculation (see supplement)
  Sigma <- solve(t(x) %*% x + diag(bold))/aold
  mu <- as.numeric(solve(t(x) %*% x + diag(bold)) %*% t(x) %*% y)
  delta <- aold*(mu^2 + diag(Sigma)) + lambda
  b <- sqrt(lambda*Calpha^2/delta)*
    ratio_besselK(sqrt(lambda*delta)*Calpha, 1) + 2/delta
  zeta <- 0.5*as.numeric(
    t(y) %*% y  - 2*t(y) %*% x %*% mu + sum(diag(t(x) %*% x %*% Sigma)) +
      t(mu) %*% t(x) %*% x %*% mu + sum(diag(diag(b) %*% Sigma)) + 
      t(mu) %*% diag(b) %*% mu)
  a <- (n + p + 1)/(2*zeta)
  e <- (b - 2/delta)*delta/(lambda*Calpha^2)
  vb <- list(delta=delta, zeta=zeta, a=a, b=b, e=e)
  
  # check
  expect_equal(vb, test.vb)
  
})

context("vb update with unpenalized covariates")
test_that("low dim. vb update", {
  
  # setting parameters
  n <- 100
  u <- 5
  r <- 10
  xu <- matrix(rnorm(n*u), nrow=n, ncol=u)
  xr <- matrix(rnorm(n*r), nrow=n, ncol=r)
  y <- rnorm(n)
  aold <- rchisq(1, 1) + 1
  bold <- rchisq(r, 1) + 1
  Calpha <- rchisq(r, 1) + 1
  lambda <- rchisq(1, 1) + 1
  yty <- as.numeric(t(y) %*% y)
  
  # implemented function
  test.vb <- .single.vb.update.unp(aold, bold, Calpha, lambda, y, xu, xr, yty, 
                                   n, u, r)
  
  # actual calculation (see supplement)
  Hold <- diag(c(rep(0, u), bold))
  x <- cbind(xu, xr)
  Sigma <- solve(t(x) %*% x + Hold)/aold
  mu <- as.numeric(solve(t(x) %*% x + Hold) %*% t(x) %*% y)
  delta <- aold*(mu[-c(1:u)]^2 + diag(Sigma)[-c(1:u)]) + lambda
  b <- sqrt(lambda*Calpha^2/delta)*
    ratio_besselK(sqrt(lambda*delta)*Calpha, 1) + 2/delta
  H <- diag(c(rep(0, u), b))
  zeta <- 0.5*as.numeric(
    t(y) %*% y  - 2*t(y) %*% x %*% mu + sum(diag(t(x) %*% x %*% Sigma)) +
      t(mu) %*% t(x) %*% x %*% mu + sum(diag(H %*% Sigma)) + 
      t(mu) %*% H %*% mu)
  a <- (n + r + u + 1)/(2*zeta)
  e <- (b - 2/delta)*delta/(lambda*Calpha^2)
  vb <- list(delta=delta, zeta=zeta, a=a, b=b, e=e)
  
  # check
  expect_equal(vb, test.vb)
  
})

test_that("high dim. vb update", {
  
  # setting parameters
  n <- 10
  u <- 5
  r <- 100
  xu <- matrix(rnorm(n*u), nrow=n, ncol=u)
  xr <- matrix(rnorm(n*r), nrow=n, ncol=r)
  y <- rnorm(n)
  aold <- rchisq(1, 1) + 1
  bold <- rchisq(r, 1) + 1
  Calpha <- rchisq(r, 1) + 1
  lambda <- rchisq(1, 1) + 1
  yty <- as.numeric(t(y) %*% y)
  
  # implemented function
  test.vb <- .single.vb.update.unp(aold, bold, Calpha, lambda, y, xu, xr, yty, 
                                   n, u, r)
  
  # actual calculation (see supplement)
  Hold <- diag(c(rep(0, u), bold))
  x <- cbind(xu, xr)
  Sigma <- solve(t(x) %*% x + Hold)/aold
  mu <- as.numeric(solve(t(x) %*% x + Hold) %*% t(x) %*% y)
  delta <- aold*(mu[-c(1:u)]^2 + diag(Sigma)[-c(1:u)]) + lambda
  b <- sqrt(lambda*Calpha^2/delta)*
    ratio_besselK(sqrt(lambda*delta)*Calpha, 1) + 2/delta
  H <- diag(c(rep(0, u), b))
  zeta <- 0.5*as.numeric(
    t(y) %*% y  - 2*t(y) %*% x %*% mu + sum(diag(t(x) %*% x %*% Sigma)) +
      t(mu) %*% t(x) %*% x %*% mu + sum(diag(H %*% Sigma)) + 
      t(mu) %*% H %*% mu)
  a <- (n + r + u + 1)/(2*zeta)
  e <- (b - 2/delta)*delta/(lambda*Calpha^2)
  vb <- list(delta=delta, zeta=zeta, a=a, b=b, e=e)
  
  # check
  expect_equal(vb, test.vb)
  
})