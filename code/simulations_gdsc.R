...
pmse <- pmset <- emse <- matrix(NA, nrow=nreps, ncol=length(methods))
...
res <- rbind(emse, pmse, pmset, cbind(est, NA, NA), cbind(elbo, NA, NA), 
             cbind(elbot, NA, NA))
...
for(r in 1:nreps) {
  ...
  est[seq(0, 4*nreps, by=nreps) + r, 1] <- 
    c(fit1.semnig$eb$alphad, rep(NA, 3), fit1.semnig$eb$lambdad)
  est[seq(0, 4*nreps, by=nreps) + r, 2] <- 
