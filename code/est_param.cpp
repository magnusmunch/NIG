...
// [[Rcpp::export]]
List est_param(arma::mat xr, arma::mat xu, arma::vec y, int n, 
               int p, double aold, arma::vec bold, 
               bool intercept, bool unpen, bool posterior) {
               // , bool start) {
  ...
  if(p <= n) {
    sigma = (x.each_col()).t()*x;
    arma::vec happend = bold;
